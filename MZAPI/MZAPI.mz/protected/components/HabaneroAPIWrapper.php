...
class HabaneroAPIWrapper {
    ...
    public function __construct($URI, $APIkey, $brandID, $currencyCode = 'PHP') {
            ...
            if (!$this->_API->GetError()) {
                if (is_array($depositResponse)) {
                    if ($depositResponse['depositmethodResult']['Success'] == true) {
                        return array('IsSucceed' => true, 'ErrorCode' => 0, 'ErrorMessage' => null, 'TransactionInfo' => $depositResponse);
                    } else {
    ...
    public function TransactionSearchInfo($RequestId) {
        ...
        if (!$this->_API->GetError()) {
            if (is_array($queryTransferResponse)) {
                if ($queryTransferResponse['querytransmethodResult']['Success'] == true) {
                    return array('IsSucceed' => true, 'ErrorCode' => 0, 'ErrorMessage' => null, 'TransactionInfo' => $queryTransferResponse);
                } else {
            ...
            if (!$this->_API->GetError()) {
                if (is_array($WithdrawPlayerMoneyResponse)) {
                    if ($WithdrawPlayerMoneyResponse['withdrawmethodResult']['Success'] == true) {
                        return array('IsSucceed' => true, 'ErrorCode' => 0, 'ErrorMessage' => null, 'TransactionInfo' => $WithdrawPlayerMoneyResponse);
                    } else {
