                ...
                if ((Utilities::validateEmail($request['Username']) || ctype_alnum($request['Username'])) && ctype_alnum($request['Password']) && is_numeric($request['AlterStr'])) {
                    ...
                    if($members) {
                        ...
                        if($alterStrLen > 16 || $alterStrLen < 14) {
                            $logMessage = 'AlterStr should only be 14-16 digits long.';
                            $transMsg = 'AlterStr should only be 14-16 digits long.';
                            $errorCode = 92;
                            Utilities::log("ReturnMessage: " . $transMsg . " ErrorCode: " . $errorCode);
                            $this->_sendResponse(200, CJSON::encode(CommonController::retMsgLogin($module, '', '', '','', $errorCode, $transMsg, '')));
                            $logger->log($logger->logdate, " [LOGIN ERROR] ", $logMessage);
...
//                                                    //var_dump('correct: '.$resultArray['OldCP']);
//                                                                                                        //var_dump('wrong: '.$resultArray['OldCurrentPoints']);
//                                                                                                        exit;
...
public function actionresetPin(){
	...
	$errorCode = 0;
	$transMsg = '';
        $txtcardnumber = trim($request['CardNumber']);
        
        $rstpin = new pcwsWrapper();
        $result = $rstpin->resetPin($txtcardnumber, 0);

        $this->_sendResponse(200, CJSON::encode(CommonController::retMsgResetPin($module, $result)));
}
