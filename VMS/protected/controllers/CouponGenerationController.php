...
class CouponGenerationController extends VMSBaseIdentity
{
            ...
            for ($i = 0; $fieldsearched > $i; $i++)
            {
                ...
                switch ($search[$i]['FieldID'])
                {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        $condition = " c.DateUpdated >= :transdatefrom";
                        break;
                    case 4:
                        $condition = " c.DateUpdated <= :transdateto AND c.Status = 3";
                        break;
                    case 5:
    ...
    public function actionGetCoupons()
    {
        ...
        $postvars['transdateto']    = $this->sanitize($_POST['transdateto']);
        ...
        if ($postvars['status'] != "" || $postvars['transdatefrom'] != ""
            || $postvars['transdateto'] != "" || $postvars['siteID'] != ""
            || $postvars['terminalID'] != "" || $postvars['source'] != ""
            || $postvars['promoname'] != "" || $postvars['couponcode'])
        {
    ...
    //        header('Content-Disposition: attachment; filename='.$filename);
            ...
            $headers = array('Batch ID', 'Coupon ID', 'Coupon Code', 'Amount', 'Distribution Type', 'Creditable?', 'Date Generated',
                             ...
                             'Source', 'Transaction Date', 'Promo Name', 'Date Reimbursed', 'Reimbursed By');
            fputcsv($fp, $headers);
            ...
            foreach ($arrcoupons as $coupons) {
                ...
                $rows = array('CouponBatchID' => $coupons['CouponBatchID'],
                              ...
                              'DateReimbursed' => $coupons['DateReimbursed'],
