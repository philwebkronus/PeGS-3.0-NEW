                        ...
                        if ($sitestat['Status'] == 1) {
                            ...
                            if (!empty($TerminalID)) {
                                ...
                                if ($TerminalID[0]['Status'] == 1) {
                                    ...
                                    $siteClass = $sitesModel->getSiteClassfication($siteID);
                                    ...
                                    if ($cnt_mapped['cnt'] > 0) {
                                        ...
                                        if (!empty($isactiveEgmSession)) {
                                            ...
                                            $stackerAmount = $stackerDetailsModel->getTotalStackerAmount($stackerBatchID);
...
//                                                
                                            ...
                                            if ($isPlaying > 0) {
                                                ...
                                                $this->_sendResponse(200, CommonController::getTerminalInfoResponse($isStarted, $isPlaying, $playingBalance, $playerMode, $currentCasino, $mappedCasinos, $minmaxAmount, $sessionMode, $membershipCardNo, $siteCode, $startDateTime, $stackerBatchID, $stackerAmount, $message, $errCode, $siteName, $siteClass));
                                            } else {
                                                if (isset($TerminalID[1]['TerminalID'])) {
                                                    ...
                                                    if ($isPlayingVIP > 0) {
                                                        ...
                                                        $membershipCardNo = $terminalSessionsModel->getLoyaltyCardNumber($TerminalID);
                                                        $startDateTime = $terminalSessionsModel->getStartDateTime($TerminalID);
                                                        $this->_sendResponse(200, CommonController::getTerminalInfoResponse($isStarted, $isPlayingVIP, $playingBalance, $playerMode, $currentCasino, $mappedCasinos, $minmaxAmount, $sessionMode, $membershipCardNo, $siteCode, $startDateTime, $stackerBatchID, $stackerAmount, $message, $errCode, $siteName, $siteClass));
                                                        ...
                                                        $startDateTime = '';
                                                        $errCode = 0;
                                                        $this->_sendResponse(200, CommonController::getTerminalInfoResponse(1, $isPlaying, $playingBalance, $playerMode, $currentCasino, $mappedCasinos, $minmaxAmount, $sessionMode, $membershipCardNo, $siteCode, $startDateTime, $stackerBatchID, $stackerAmount, $message, $errCode, $siteName, $siteClass));
                                                    }
                                                } else {
                                                    $message = "Cannot find Terminal.";
                                                    $errCode = 3;
                                                    $this->_sendResponse(200, CommonController::getTerminalInfoResponse(0, $isPlaying, $playingBalance, $playerMode, $currentCasino, $mappedCasinos, $minmaxAmount, $sessionMode, $membershipCardNo, $siteCode, $startDateTime, $stackerBatchID, $stackerAmount, $message, $errCode, $siteName, $siteClass));
                                                }
                                            }
                                        } else {
                        ...
                        $message = "Cannot find Terminal.";
                        $errCode = 3;
                        $this->_sendResponse(200, CommonController::getTerminalInfoResponse(0, $isPlaying, $playingBalance, $playerMode, $currentCasino, $mappedCasinos, $minmaxAmount, $sessionMode, $membershipCardNo, $siteCode, $startDateTime, $stackerBatchID, $stackerAmount, $stackerAmount, $message, $errCode, $siteName, $siteClass));
                    }
                } else {
...
//        else if (((!isset($request['TerminalName']) && $request['TerminalName']) == '') && ((!isset($request['MembershipCardNumber']) && $request['MembershipCardNumber']) == '')) {
    ...
    public function actionStartSession() {
        ...
        if (isset($casinoID) && $casinoID != '' && isset($terminalName) && $terminalName != '' && isset($playerMode) && $playerMode != '' && isset($cardNumber) && $cardNumber != '' && isset($amount) && $amount != '' && isset($trackingID) && $trackingID != '' && isset($stackerBatchID) && $stackerBatchID != '') {
            ...
            //check if e-safe or reg. e-SAFE not allowed
            $isEsafe = $membersModel->checkIfEwallet($mid);
            if ($isEsafe['IsEwallet'] == 1) {
                $message = "e-SAFE account is not allowed to start a session.";
                $this->_sendResponse(200, CommonController::startSessionResponse(2, $DateTime, '', $message, 74));
...
//                        }
                ...
                $loyaltyconfig = Yii::app()->params['isLoyalty'];
                if ($loyaltyconfig == 1)
                {
...
//                    }
    ...
    public function actionRedeemSession() {
        ...
        if (isset($terminalName) && $terminalName != '' && isset($trackingID) && $trackingID != '' && isset($stackerBatchID) && $stackerBatchID != '') {
            ...
            if (!empty($lastsessiondetails)) {
                ...
                $isEsafe = $membersModel->checkIfEwallet($mid);
                if ($isEsafe['IsEwallet'] == 1) {
                    $message = "e-SAFE account is not allowed.";
                    $this->_sendResponse(200, CommonController::redeemSessionResponse(2, '', '', '', '', '', '', '', $message, 0, 74));
    ...
    public function actionReloadSession() {
        ...
        if (isset($terminalName) && $terminalName != '' && isset($amount) && $amount != '' && isset($trackingID) && $trackingID != '' && isset($stackerBatchID) && $stackerBatchID != '') {
            ...
            if ($isEsafe['IsEwallet'] == 1) {
                $message = "e-SAFE account is not allowed.";
                $this->_sendResponse(200, CommonController::reloadSessionResponse(2, $DateTime, '', $message, 74));
    ...
    /**
     * e-SAFE Reload Genesis Controller
     * @author Mark Kenneth Esguerra
    ...
    public function actionEsafegenesisreload(){
        ...
        if (isset($request['TerminalName']) && isset($request['Amount']) && 
            ...
            //sanitiza inputs
            $terminalName   = htmlentities(trim($request['TerminalName']));
            $trackingID     = htmlentities(trim($request['TrackingID']));
            $amount         = htmlentities(trim($request['Amount']));
            $stackerBatchID = htmlentities(trim($request['StackerBatchID']));
            
            if ($terminalName != "" && $trackingID != "" && 
                $amount != "" && $stackerBatchID != "") {
                
                if (Utilities::validateInput($terminalName) && Utilities::validateInput($trackingID) && 
                    Utilities::validateInput($amount) && Utilities::validateInput($stackerBatchID)) {
                    ...
                    //check if the site has a virtual cashier
                    $this->acc_id = $siteAccountsModel->getVirtualCashier($siteid);
                    if ($this->acc_id == false) {
                        $message = "Terminal has no virtual cashier. Please contact Philweb Customer Service Hotline 338-3388.";
                        $this->_sendResponse(200, CommonController::eSafeReloadGenResponse(2, $DateTime, '', $message, 71));

                        exit;
                    }
                    
                    $lastsessiondetails = $terminalSessionsModel->getLastSessionDetails($terminalidreg, $terminalidvip);
                    //check if has terminal session
                    if (!empty($lastsessiondetails)) {
                        $terminalid = $lastsessiondetails['TerminalID'];
                        $casinoUsername = $lastsessiondetails['UBServiceLogin'];
                        $casinoPassword = $lastsessiondetails['UBServicePassword'];
                    ...
                    if ($isEwallet['IsEwallet'] != 1) {
                     ...
                     //Check Last reload transaction
                    $stackerReload = $stackerDetailsModel->getLastReloadTrans($stackerBatchID);

                    //Check if entered amount is equal to amount in stacker details
                    if ($amount != $stackerReload['Amount']) {
                        $message = 'Invalid Amount.';
                        $this->_sendResponse(200, CommonController::eSafeReloadGenResponse(2, $DateTime, '', $message, 23));
                    ...
                    $trans_id = $gamingRequestLogsModel->insertGamingRequestLogs($trackingID, $amount, '0', 'R', $siteid, $terminalid, $casinoServiceID, $loyaltyCardNo, $mid);
                    if ($trans_id > 0) {
                        //GET last reload
                        $total = $stackerDetailsModel->getLastReloadTrans($stackerBatchID);
                        $paymentType = $total['PaymentType'] + 1;
                        //check if ticket is cash or ticket
                        if (!is_numeric($amount)) {
                            $bool = substr($amount, 0, 1) === 'T';
                            //if the input starts with 'T' then it might be a ticket, if not, throw error
                            if (!$bool) {
                                $message = "Amount should be numeric or a Ticket";
                                $this->_sendResponse(200, CommonController::eSafeReloadGenResponse(2, $DateTime, '', $message, 22));
                            ...
                            if (isset($verifyVoucherResult['VerifyTicket']['ErrorCode']) && $verifyVoucherResult['VerifyTicket']['ErrorCode'] == 0) {
                                ...
                                if (isset($verifyVoucherResult['VerifyTicket']['Amount']) && $verifyVoucherResult['VerifyTicket']['Amount'] != '') {
                                    ...
                                    if ($casinoUserMode == 0) {
                                        $login_acct = $terminalName;
                                        $terminal_pwd = $terminalsModel->getTerminalPassword($terminalid, $casinoServiceID);

                                        if ($terminal_pwd['ServicePassword'] == '') {
                                            $message = 'There are no mapped casino in this terminal';
                                            $this->_sendResponse(200, CommonController::eSafeReloadGenResponse(2, $DateTime, '', $message, 49));
                                        ...
                                        $sitebalance = Utilities::toMoney($sitebalance['Balance']);
                                        if ($total['Amount'] == $amount) {
                                            $result = $terminalBasedTrans->reload(Utilities::toInt($sitebalance), $amount, $paymentType, $stackerBatchID, $terminalid, $siteid, $casinoServiceID, $this->acc_id, $loyaltyCardNo, $vouchercode, $trackingID, $mid, $casinoUserMode, $stackerReload['StackerDetailID']);
                                        } else {
                                            $message = "Invalid Amount.";
                                            $this->_sendResponse(200, CommonController::eSafeReloadGenResponse(2, $DateTime, '', $message, 60));
                                    ...
                                    if ($casinoUserMode == 1) {
                                        ...
                                        $sitebalance = Utilities::toMoney($sitebalance['Balance']);
                                        if ($total['Amount'] == $amount) {
                                            $result = $userBasedTrans->eSAFEReload(Utilities::toInt($sitebalance), $amount, $paymentType, $stackerBatchID, $terminalid, $siteid, $casinoServiceID, $this->acc_id, $loyaltyCardNo, $vouchercode, $trackingID, $mid, $casinoUserMode, 
                                                                                   $casinoUsername, $casinoPassword, $casinoServiceID, $stackerReload['StackerDetailID']);
                                        } else {
                                            $message = "Invalid Amount.";
                                            $this->_sendResponse(200, CommonController::eSafeReloadGenResponse(2, $DateTime, '', $message, 60));
                            ...
                            if ($casinoUserMode == 0) {
                                ...
                                if ($terminal_pwd['ServicePassword'] == '') {
                                    ...
                                    $gamingRequestLogsModel->updateGamingLogsStatus($trans_id, $this->status, null, null, null);
                                    $message = 'There are no mapped casino in this terminal';
                                    $this->_sendResponse(200, CommonController::eSafeReloadGenResponse(2, $DateTime, '', $message, 47));
                            ...
                            if ($casinoUserMode == 1) {
                                ...
                                //Check total reload is equal to entered amount
                                if ($total['Amount'] == $amount) {
                                    $result = $userBasedTrans->eSAFEReload(Utilities::toInt($sitebalance), $amount, $paymentType, $stackerBatchID, $terminalid, $siteid, $casinoServiceID, $this->acc_id, $loyaltyCardNo, $vouchercode, $trackingID, $mid, $casinoUserMode, $casinoUsername, $casinoPassword, $casinoServiceID, $stackerReload['StackerDetailID']);
                                } else {
                                    $message = "Invalid Amount.";
                                    $this->_sendResponse(200, CommonController::eSafeReloadGenResponse(2, $DateTime, '', $message, 60));
                        ...
                        if (isset($result['ewallet_trans_id'])) {
                            $this->status = $result['transStatus'];
                            //get cashier user
                            $user = $siteAccountsModel->getAIDByAccountTypeIDAndTerminalID(15, $terminalid);
                            //Update Stacker Summary Status
                            $updatestat = $stackerSummaryModel->updateStackerSummaryStatus($stackerBatchID, StackerSummaryModel::STATUS_RELOAD, $user);
                            ...
                            $gamingRequestLogsModel->updateGamingLogsStatus($trans_id, $this->status, null, null, null);
                            $this->_sendResponse(200, CommonController::eSafeReloadGenResponse(2, '', '', $result['TransMessage'], $result['ErrorCode']));
                        }
                        ...
                        if (!isset($result["ewallet_trans_id"])) {
                            ...
                            $trans_details_id = $result["ewallet_trans_id"];
                        }
                        ...
                        //Check if Loyalty Points is ON or OFF
                    $loyaltyconfig = Yii::app()->params['isLoyalty'];
                    if ($loyaltyconfig == 1)
                    {     //Insert to loyaltyrequestlogs
                        $loyaltyrequestlogsID = $loyaltyrequestlogs->insert($mid, 'R', $terminalid, $amount, $trans_details_id, 0, $isCreditable);
                    ...
                    $message = "Parameters are not set";
                    $this->_sendResponse(200, CommonController::eSafeReloadGenResponse(2, $DateTime, '', $message, 11));
                }
            }
            else {
                $message = "Parameters are not set.";
    ...
    public function actionEsafegenesisredemption() {
        ...
        if (isset($terminalName) && $terminalName != '' && isset($trackingID) && $trackingID != '' && isset($stackerBatchID) && $stackerBatchID != '') {
            ...
            if ((!Utilities::validateInput($terminalName)) || (!Utilities::validateInput($trackingID))) {
                ...
                $message = 'Parameters contains invalid special characters';
                $this->_sendResponse(200, CommonController::eSafeRedemptionGenResponse(2, '', '', '', '', '', '', '', $message, 0, 21));
            ...
            if (!is_numeric($stackerBatchID)) {
                $message = "Invalid Stacker Batch ID";
                $this->_sendResponse(200, CommonController::eSafeRedemptionGenResponse(2, '', '', '', '', '', '', '', $message, 0, 51));
            ...
            if ($this->acc_id == false) {
                $message = "Terminal has no virtual cashier. Please contact Philweb Customer Service Hotline 338-3388.";
                $this->_sendResponse(200, CommonController::eSafeRedemptionGenResponse(2, '', '', '', '', '', '', '', $message, 0, 71));
            ...
            if (!empty($lastsessiondetails)) {
                ...
                if(is_string($getBalance)){
                    ...
                    Utilities::errorLogger($message, "Redeem Session", 
                                            ...
                                            "SiteID: ".$siteid." | ".
                                            "CasinoID: ".$casinoServiceID);
                    $this->_sendResponse(200, CommonController::eSafeRedemptionGenResponse(2, '', '', '', '', '', '', '', $message, 0, 26));
                }
                list($terminal_balance,$service_name,$terminalSessionsModel,
                        $transReqLogsModel,$redeemable_amount,$casinoApiHandler,$mgaccount) = $getBalance;
            ...
            if ($isEwallet['IsEwallet'] != 1) {
                $message = "Non-eSAFE player is not allowed.";
                $this->_sendResponse(200, CommonController::eSafeRedemptionGenResponse(2, '', '', '', '', '', '', '', $message, 0, 73));
            ...
            if ($countMatchedID == 0) {
                $message = "Terminal and StackerBatchID does not match in EGM session.";
                $this->_sendResponse(200, CommonController::eSafeRedemptionGenResponse(2, '', '', '', '', '', '', '', $message, 0, 42));
            
                exit;
            }
            //check if the entered partial amount is less than or equal to redeemable amount
            if ($partialAmount > $redeemable_amount) {
            ...
            if(!isset($voucherTicketBarcode) || $voucherTicketBarcode == "") { 
                $message = "Failed to generate ticket.";
                $this->_sendResponse(200, CommonController::eSafeRedemptionGenResponse(2, '', '', '', '', '', '', '', $message, 0, 62));
            ...
            $card_number = $loyaltyCardNo;
            //check if tracking ID is unique
            $trans_id = $gamingRequestLogsModel->insertGamingRequestLogs($trackingID, $partialAmount, '0', 'W', $siteid, $terminalid, $casinoServiceID, $card_number, $mid);
            //check if tracking ID is unique in tickets table
            $isTrackIDExist = $tickets->checkIfTrackingIDExist($trackingID);
            ...
            if ($trans_id != false && $isTrackIDExist == false) {
                ...
                if ($casinoUserMode == 1) {
                    ...
                    $result = $userBasedTrans->eSAFERedemption($login_pwd, $terminalid, $login_acct, $stackerBatchID, $siteid
                            , Utilities::toInt($sitebalance), $casinoServiceID
                            , $redeemable_amount, $paymentType, $this->acc_id, $loyaltyCardNo, $mid, $casinoUserMode, $login_acct, $trackingID
                            , $voucherTicketBarcode, $casinoPassword, $partialAmount);
                }
                ...
                if (!isset($result['terminal_name'])) {
                    ...
                    //Check if Loyalty is ON or OFF
                    $loyaltyconfig = Yii::app()->params['isLoyalty'];
                    if ($loyaltyconfig == 1)
                    {   //Insert to loyaltyrequestlogs
                        $loyaltyrequestlogsID = $loyaltyrequestlogs->insert($mid, 'W', $terminalid, $partialAmount, $trans_details_id, 0, $isCreditable);
                        $transdate = CasinoApi::udate('Y-m-d H:i:s.u');

                        $isSuccessful = $loyalty->processPoints($loyaltyCardNo, $transdate, 1, 'W', $partialAmount, $siteid, $trans_details_id, $terminal_name, $isCreditable, '', 7, 1);
                ...
                if (isset($result['transStatus']) && $result['transStatus'] == 1) {
                    ...
                    if ($result['amount'] > 0) {
                        ...
                        if (isset($result['trans_summary_id'])) {
                            ...
                            $this->status = 2;
                            $gamingRequestLogsModel->updateGamingLogsStatus($trans_id, $this->status, null, null, null);
                            $this->_sendResponse(200, CommonController::eSafeRedemptionGenResponse(1, $result['amount'], '', $result['TransactionDate'], '', $sitecode, $loyaltyCardNo, $result['ewallet_trans_id'], $result['TransMessage'], 0, 0));
                        }
                    } else {
                        $this->status = 1;
                    ...
                    $gamingRequestLogsModel->updateGamingLogsStatus($trans_id, $this->status, null, null, null);
                    $this->_sendResponse(200, CommonController::eSafeRedemptionGenResponse(2, $amount,  '', '', '', $sitecode, '', '', $result['TransMessage'], 0, $result['ErrorCode']));
                }
            } else {
                $this->status = 2;
