...
class MemberCards extends BaseEntity {
    ...
    /**
     * Get Member's Info (FirstName, LastName, Email and UB Card)by UB Card
     * @param type $UBcard UB card of the player
     * @return array Member's Info
     * @author Mark Kenneth Esguerra
     * @date October 9, 2013
     */
    public function getMemberInfoByCard($UBcard)
    {
        $query = "SELECT mc.MID, mm.UserName, mc.CardNumber, mi.FirstName, mi.LastName, mi.Email
                  FROM $this->TableName mc 
                  INNER JOIN membership.members mm ON mc.MID = mm.MID
                  INNER JOIN membership.memberinfo mi ON mm.MID = mi.MID
                  WHERE mc.CardNumber = '$UBcard' AND mc.Status = 1
                ";
        $result = parent::RunQuery($query);
    ...
    //For updating player points
    /**
     * Update Player Points (ADD)
     * @param $MID Member ID
     * @param $redeemTotalPoints Redeemed Points
     * @return boolean
     */
    public function updatePlayerPointsAdd($MID, $redeemTotalPoints) {
        $query = "UPDATE $this->TableName set RedeemedPoints = RedeemedPoints - $redeemTotalPoints, 
                CurrentPoints = CurrentPoints + $redeemTotalPoints WHERE MID = $MID AND  Status IN (1,5)";
        return parent::ExecuteQuery($query);
    }
    
    public function counterTempAccount($UBcard)
    {
        $query = "SELECT COUNT(MemberCardID) as ctrtemp FROM $this->TableName 
                  WHERE Status IN (5, 8) AND CardNumber = '$UBcard'
                ";
        $result = parent::RunQuery($query);
