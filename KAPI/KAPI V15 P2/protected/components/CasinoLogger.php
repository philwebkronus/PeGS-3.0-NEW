...
class CasinoLogger {
    ...
    function apirequestlog($message, $type, $newLine = false) {
        ...
        if (!file_exists($log_path)) {
            $create_file = fopen($log_path, "w+"); //create the new file
            chmod($log_path, 0777); //set the appropriate permissions.
            fclose($create_file);
        }
        if (file_put_contents($log_path, $message, FILE_APPEND) === false){
            throw new Exception($log_path . ' is not writable');
        }
    }
