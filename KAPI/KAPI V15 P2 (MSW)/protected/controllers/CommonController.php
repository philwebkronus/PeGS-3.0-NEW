...
//        Yii::import('application.components.CasinoApi2');
    ...
    public static function getTerminalInfoResponse($isStarted, $isPlaying, $playingBalance, $playerMode, $currentCasino,
            ...
            $stackerBatchID, $stackerAmount, $transMsg,$errCode,$siteName, $siteClass){
        ...
        return CJSON::encode(array('GetTerminalInfo'=>(array('IsStarted'=>(int)$isStarted,'IsPlaying'=>(int)$isPlaying,'PlayingBalance'=>$playingBalance,
                                   ...
                                   'MaxRedeemableAmount' => (float)Yii::app()->params['MaxRedeemableAmount'], 'SiteClassificationID' => $siteClass,
     ...
     * @date 9/21/15
     */
    public static function eSafeReloadGenResponse($status, $DateTime, $trackingID, $transMsg, $errCode){

        return CJSON::encode(array('eSAFEGenesisReload'=>(array('Status'=>(int)$status,
                                               'DateTime'=>$DateTime,
                                               'TransactionID'=>$trackingID,
                                               'TransactionMessage'=>$transMsg,
                                               'ErrorCode'=>(int)$errCode))));
    ...
    public static function eSafeRedemptionGenResponse($status,$amount, $VoucherTicketBarcode, $DateTime, $ExpirationDate,
            $SiteCode, $membershipcardno, $trackingID, $transMsg, $isPrintTicket, $errCode){

        return CJSON::encode(array('eSAFEGenesisRedemption'=>(array('Status'=>(int)$status,
                                               'Amount'=>$amount,
                                               'VoucherTicketBarcode'=>$VoucherTicketBarcode,
                                               'DateTime'=>$DateTime,
                                               'ExpirationDate'=>$ExpirationDate,
    ...
    /*
     * @description: outputs json response for Get Balance MSW module
     * @author: ralph sison
     * @dateadded: 12-22-2015
     */
    public static function getBalanceMSW($balance, $transMsg, $errCode) {
        return CJSON::encode(array('GetBalanceMSW' => (array('Balance' => $balance,
                                                             'TransactionMessage' => $transMsg,
                                                             'ErrorCode' => (int) $errCode))));
    }

    /*
     * @description: outputs json response for Deposit MSW module
     * @dateadded: 12-28-2015
     */
    public static function depositMSW($transID, $transStatus, $transMsg, $errCode) {
        return CJSON::encode(array('DepositMSW' => (array('TransactionID' => $transID,
    ...
    /*
     * @description: outputs json response for Withdraw MSW module
     */
    public static function withdrawMSW($transID, $transStatus, $transMsg, $errCode) {
        return CJSON::encode(array('WithdrawMSW' => (array('TransactionID' => $transID,
                                                          'TransactionStatus' => $transStatus,
                                                          'TransactionMessage' => $transMsg,
                                                          'ErrorCode' => (int) $errCode))));
    }
