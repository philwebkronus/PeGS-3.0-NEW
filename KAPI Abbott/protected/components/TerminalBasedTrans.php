...
public function start($terminal_id,$site_id,$trans_type,$service_id,$bcf,$initial_deposit,$acctid,
        ...
        //check if terminal has an existing balance
        ...
        if(!$is_terminal_exist){
            ...
            Utilities::errorLogger($message, "Start Session", 
                                    ...
                                    "SiteID: ".$site_id." | ".
                                    "CasinoID: ".$service_id);
            return array('TransMessage'=>$message,'ErrorCode'=>37);
        }
        
        //insert into transaction request log
        $trans_req_log_last_id = $transReqLogsModel->insert2($udate, $initial_deposit, 'D', $terminal_id, $site_id, $service_id, 0, $loyalty_card, $mid, $userMode, $stackerbatchid, '', '', $transaction_id);
        ...
        if ($apiresult == "true" || $apiresult == 'TRANSACTIONSTATUS_APPROVED' || $apiresult == 'approved'){
            ...
            Utilities::errorLogger($message, "Start Session", 
                                    ...
                                    "SiteID: ".$site_id." | ".
                                    "CasinoID: ".$service_id);
            return array('TransMessage'=>$message,'ErrorCode'=>44);
        }
     ...
     public function reload($bcf,$amount,$paymentType,$stackerbatchid, $terminal_id,$site_id,$service_id,$acctid,
            $loyalty_card, $voucher_code = '',$trackingid = '', $mid = '', $userMode = '', $stackerdetailID = '') {
        Yii::import('application.components.CasinoApiUB');
        ...
        if($is_terminal_active < 1) {
            ...
            Utilities::errorLogger($message, "Reload Session", 
                                    ...
                                    "SiteID: ".$site_id." | ".
                                    "CasinoID: ".$service_id);
            return array('TransMessage'=>$message,'ErrorCode'=>31);
        }
        ...
        if(strpos($service_name, 'MG') !== false) {
            ...
            if(!$transaction_id){
                ...
                Utilities::errorLogger($message, "Reload Session", 
                                    ...
                                    "SiteID: ".$site_id." | ".
                                    "CasinoID: ".$service_id);
                return array('TransMessage'=>$message,'ErrorCode'=>36,'DateExpiry'=>'');
            }
        } else {
            $transaction_id = '';
        }
        ...
        if(!$trans_req_log_last_id) {
            ...
            Utilities::errorLogger($message, "Reload Session", 
                                    ...
                                    "SiteID: ".$site_id." | ".
                                    "CasinoID: ".$service_id);
            return array('TransMessage'=>$message,'ErrorCode'=>38,'DateExpiry'=>'');
        }
        ...
        if ($apiresult == "true" || $apiresult == 'TRANSACTIONSTATUS_APPROVED' || $apiresult == 'approved'){
            ...
            //this will return the transaction summary ID as well as transaction id
            $trans_details_id = $commonTransactionsModel->reloadSessionTransaction($amount, $trans_summary_id, $udate, $site_id, $terminal_id, 
                                    'R', $paymentType, $service_id, $acctid, $transstatus, $total_reload_balance, $total_terminal_balance, $loyalty_card, $mid, $stackerbatchid);
     ...
     public function redeem($login_pwd, $terminal_id, $terminal_name, $stackerbatchid, $site_id,$bcf,
             $service_id,$amount, $paymentType,$acct_id,
            $loyalty_card, $trackingID, $voucherTicketBarcode = '', $mid = '', $userMode = '') {
        Yii::import('application.components.CasinoApiUB');
...
//        $casinoApi->callSpyderAPI($commandId = 1, $terminal_id, $terminal_name, $login_pwd, $service_id, $spyder_enabled);
        
        //call PT, freeze and force logout of session
        $casinoApi->_doCasinoRules($terminal_id, $service_id, $terminal_name);
...
//            $casinoApi->callSpyderAPI($commandId = 0, $terminal_id, $terminal_name, $login_pwd, $service_id, $spyder_enabled);
        ...
        if($redeemable_amount > 0){
                ...
                /************************* WITHDRAW ************************************/
                $resultwithdraw = $casinoApiHandler->Withdraw($terminal_name, $amount, $tracking1, $tracking2, 
                              $tracking3, $tracking4,  $login_pwd, $event_id, $transaction_id);
...
//                        $casinoApi->callSpyderAPI($commandId = 0, $terminal_id, $terminal_name, $login_pwd, $service_id, $spyder_enabled);
            ...
            return array('TransMessage'=>'Info: Session has been ended.','transStatus'=>1,
                        'trans_summary_id'=>$trans_summary_id,'udate'=>$udate,'amount'=>$amount,'terminal_login'=>$terminal_name,
