...
class LPTerminalServices extends LPModel
{
    ...
    {
        $query = 'SELECT ts.HashedServicePassword, ts.HashedServicePassword as ServicePassword,ts.ServiceID, rs.Code 
    ...
    public function getServiceID($terminalID)
    {
        $query = "SELECT ServiceID FROM npos.terminalservices "
    ...
    public function getServiceGroupID($terminalCode)
    {
        
        $query = "SELECT rs.ServiceGroupID "
                . "FROM terminals t "
                . "INNER JOIN terminalservices tss ON tss.TerminalID = t.TerminalID "
                . "INNER JOIN ref_services rs ON rs.ServiceID = tss.ServiceID "
                . "WHERE t.TerminalCode IN ('$terminalCode','$terminalCode"."VIP') "
                . "AND tss.Status = 1 "
                . "GROUP BY rs.ServiceGroupID";
        ...
        if(!$result) {
            $this->logerror("File: launchpad.models.LPTerminalServices, Message: Can't get terminal service ID");
        }
        return $result;
