...
class LobbyController extends CController 
{
    ...
    public function actionIndex()
    {
        if(Yii::app()->user->isGuest) {
            ...
            //Get the current User Mode of Current Casino
            $isUserBased = LPRefServices::model()->getUserMode($this->getCurrentServiceID());
            
            //Check the User Mode of Current Casino and get the appropriate credentials.
            if($isUserBased == 1){
                $TerminalCode = $this->getTerminalCode();
                $MID = LPTerminalSessions::model()->getMID($TerminalCode);
                $logincredentials = LPMemberServices::model()->GetUBCredentials($this->getCurrentServiceID(), '', $MID);
                ...
                Yii::app()->user->setState('UBUsername', $login);
            } else {
            ...
            Yii::app()->user->setState('terminalPwd', $password);
            Yii::app()->user->setState('encryptPwd', $hashedpassword);
    ...
    public function actionTransfer()
    {
        ...
        if(!LPTransactionRequestLogsLp::model()->insert($currentBalance, $lastServID, 'D', 
                $pickServiceID, $loyaltyCardNo, $mid, $duserMode,$lpTransactionID)) {
        ...
        LPTerminalSessions::model()->updateCurrentServiceID($this->getTerminalID(), $pickServiceID, $password, $hashedpassword, $servicelogin, $duserMode);
    ...
    protected function withdraw($lpTransactionID, $serviceID,$serviceType,$amount, array $tracking, $casinoApiHandler, $currentbet = 0)
    {
        ...
        if($isUserBased == 0){
            if($serviceType == "rtg2"){
                $TerminalCode = $this->getTerminalCode();
                $logincredentials = LPTerminalServices::model()->getTBCredentials($TerminalCode, $serviceID);
                $login = $TerminalCode;
                $plainPassword = $logincredentials["ServicePassword"];
            } else {
                $login = $this->getTerminalCode();
        ...
        if($isUserBased == 1){
            $logincredentials = LPMemberServices::model()->GetUBCredentials($serviceID, $this->getUBLogin());
            $login = $logincredentials["ServiceUsername"];
            $plainPassword = $logincredentials["ServicePassword"];
        }
...
//                }
    ...
    protected function deposit($lpTransactionID, $serviceID,$serviceType,$amount, array $tracking, $casinoApiHandler) {
        ...
        //Prepare credentials for accessing casino api in deposit
        $isUserBased = LPRefServices::model()->getUserMode($serviceID);
        if($isUserBased == 0){
            if($serviceType == "rtg2"){
                $TerminalCode = $this->getTerminalCode();
        ...
        switch($serviceType) {
           ...
           case 'pt':
               ...
               $changeStatusResult = $casinoApiHandler->ChangeAccountStatus($login, 0);
               
               
               $depositResult = $casinoApiHandler->Deposit($login, Lib::moneyToDecimal($amount), 
                                                    $tracking[0], $tracking[1], 
                                                    $tracking[2], $tracking[3], $plainPassword, 
                                                    $eventid, $lpTransactionID);
