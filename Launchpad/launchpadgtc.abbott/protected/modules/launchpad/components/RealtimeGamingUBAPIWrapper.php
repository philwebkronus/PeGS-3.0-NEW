...
Yii::import('application.modules.launchpad.components.casinoapi.RealtimeGamingCashierAPI');
Yii::import('application.modules.launchpad.components.casinoapi.RealtimeGamingCasinoGamesAPI');
...
class RealtimeGamingUBAPIWrapper
{
    ...
    public function __construct( $URI, $API, $certFilePath, $keyFilePath )
    {
        switch ($API)
        {
            ...
            case self::GAME_API:
                {
                    $this->_API = new RealtimeGamingCasinoGamesAPI( $URI, $certFilePath, $keyFilePath, '' ); break;
            ...
            if ( $response[ 'IsSucceed' ] == true )
            {
                ...
                if ( !is_null( $response ) )
                {
                    if ( $response[ 'IsSucceed' ] == true )
                    {
                        ...
                        $hashedPassword = $accountInfo[ 'GetAccountInfoByPIDResult' ][ 'hashedPassword' ];
