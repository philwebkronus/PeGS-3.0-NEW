                    ...
                    if(Utilities::validateEmail($emailCardNumber)) {
                        ...
                        if($data) {
                            ...
                            if($result > 0) {
                                ...
                                $isSuccessful = $auditTrailModel->logEvent(AuditTrailModel::FORGOT_PASSWORD, $emailCardNumber, array('MID' => $MID));
                                if($isSuccessful == 0) {
...
//                                }
    ...
    public function actionUpdateProfile() {
        ...
        && isset($request['EmailAddress']) && isset($request['IDNumber']) && isset($request['Birthdate']) && isset($request['MPSessionID'])) {
            ...
            || ($request['IDNumber'] == '') || ($request['Birthdate'] == '') || ($request['MPSessionID'] == '')) {
            ...
            else if(!Utilities::validateNumeric($request['MobileNo']) || !Utilities::validateNumeric($request['AlternateMobileNo'])) {
                $transMsg = "Mobile number should consist of numbers only.";
                $errorCode = 16;
                Utilities::log("ReturnMessage: " . $transMsg . " ErrorCode: " . $errorCode);
                $this->_sendResponse(200, CJSON::encode(CommonController::retMsgUpdateProfile($module, $transMsg, $errorCode)));
                ...
                Utilities::log("ReturnMessage: " . $transMsg . " ErrorCode: " . $errorCode);
                $this->_sendResponse(200, CJSON::encode(CommonController::retMsgUpdateProfile($module, $transMsg, $errorCode)));
                $logMessage = 'Password should be the same as confirm password.';
                $logger->log($logger->logdate, " [UPDATEPROFILE ERROR] ", $logMessage);
                $apiDetails = 'UPDATEPROFILE-Failed: Invalid input parameters. ';
                $isInserted = $apiLogsModel->insertAPIlogs($apiMethod, '', $apiDetails, '', 2);
                if($isInserted == 0) {
...
//                }
                ...
                $gender = trim($request['Gender']);
                $birthdate = trim($request['Birthdate']);
                $nationalityID = trim($request['NationalityID']);
                $occupationID = trim($request['OccupationID']);
                $isSmoker = trim($request['IsSmoker']);
                $mpSessionID = trim($request['MPSessionID']);
...
//                    exit;
                    ...
                    for($itr = 0; $itr < count($rewardoffers); $itr++) {
                        ...
                        $thumbnailLimitedImage = $rewardoffers[$itr]["ThumbnailLimitedImage"];
                        $eCouponImage = $rewardoffers[$itr]["ECouponImage"];
                        $learnMoreLimitedImage = $rewardoffers[$itr]["LearnMoreLimitedImage"];
                        $learnMoreOutOfStockImage = $rewardoffers[$itr]["LearnMoreOutOfStockImage"];
                        $thumbnailOutOfStockImage = $rewardoffers[$itr]["ThumbnailOutOfStockImage"];
                        $promoName = $rewardoffers[$itr]["PromoName"];
                        $isMystery = $rewardoffers[$itr]["IsMystery"];
                        $mysteryName = $rewardoffers[$itr]["MysteryName"];
                        $mysteryAbout = $rewardoffers[$itr]["MysteryAbout"];
                        $mysteryTerms = $rewardoffers[$itr]["MysteryTerms"];
                        $mysterySubtext = $mysteryAbout = $rewardoffers[$itr]["MysterySubtext"];
    ...
    public function actionRedeemItems() {
        ...
        if(isset($request['CardNumber']) && isset($request['RewardItemID']) && isset($request['Quantity']) && isset($request['ItemQuantity']) 
          && isset($request['RedeemedPoints']) && isset($request['RewardID']) && isset($request['Source']) && isset($request['RedeemedDate']) && isset($request['MPSessionID'])) {
            if(($request['CardNumber'] == '') || ($request['RewardItemID'] == '') || ($request['RewardID'] == '') || ($request['Quantity'] == '')
            || ($request['ItemQuantity'] == '') || ($request['RedeemPoints'] == '') || ($request['Source'] == '') || ($request['RedeemDate'] == '') || ($request['MPSessionID'] == '')) {
                $transMsg = "One or more fields is not set or is blank.";
                $errorCode = 1;
                Utilities::log("ReturnMessage: " . $transMsg . " ErrorCode: " . $errorCode);
                $this->_sendResponse(200, CJSON::encode(CommonController::retMsg($module, $transMsg, $errorCode)));
                ...
                if($isExist > 0) {
                    if($MID == '' || $MID == null) {
                        ...
                        if($memberDetails) {
                            ...
                            $result = $auditTrailModel->logEvent(AuditTrailModel::GET_PROFILE, 'GetProfile', array('ID' => $MID, 'SessionID' => $mpSessionID));
                            if($result == 0) {
                               $logMessage = "Failed to insert to Audittrail.";
                               $logger->log($logger->logdate, " [GETPROFILE ERROR] ", $logMessage); 
                            }
                            
                            $profile = array($firstname, $middlename, $lastname, $nickname, $permanentAddress, $mobileNumber, $alternateMobileNumber, $emailAddress, $alternateEmail, $gender, $idPresented, $idNumber, $nationality, $occupation, $isSmoker, $birthDate);
                            $transMsg = 'No Error, Transaction successful.';
                            $errorCode = 0;
