...
class BTAmpapiController extends Controller {
    ...
    private $ApiMethodID = array(
        ...
        'RegisterMemberBTNoEmail'=>APILogsModel::API_REGISTER_MEMBER_BT_NO_EMAIL
    );
	...
	public function actionRegisterMemberBTNoEmail(){
       ...
       $fields = array('TPSessionID'=>false,'FirstName'=>false,'LastName'=>false,'MobileNo'=>false,'Birthdate'=>false);

       $validateRequiredFields = $this->validateRequiredFieldsRBT($request, $module, $fields);
       if($validateRequiredFields ===true){
           $this->_registerMemberBTNoEmail($request, $module);
       }
     ...
     * @javida
     */
    private function _registerMemberBTNoEmail($request, $module){
        $TPSessionID = trim($request['TPSessionID']);
        ...
        if($validateTPSessionID===true){
            ...
            if(isset($result[0]) && $result[0]==200){
                ...
                $this->_displayCustomMessages($module, 73, 'No response from Membership portal API.');
                $this->_apiLogs(APILogsModel::API_REGISTER_MEMBER_BT,'' , 73, '', 2, $module, $LastName.', '.$FirstName);
            }
        }
    }
