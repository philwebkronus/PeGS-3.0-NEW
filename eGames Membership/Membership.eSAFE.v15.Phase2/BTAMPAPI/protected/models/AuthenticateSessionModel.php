...
class AuthenticateSessionModel {
    ...
    public function authenticateCredentials($Username, $Password){
        $sql = 'SELECT COUNT(`AID`) as Count,`AID`, `Username`, `Password`, `Status` FROM `accounts` WHERE `Username`=:Username AND `Password`=:Password AND `Status`=:Status AND UserAccessTypeID = 2 LIMIT 1';
