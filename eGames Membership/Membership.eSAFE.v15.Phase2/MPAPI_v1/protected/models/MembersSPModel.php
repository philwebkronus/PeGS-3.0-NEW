...
class MembersSPModel {
    ...
    public static function model()
    {
        if(self::$_instance == null)
            self::$_instance = new MembersSPModel();
    ...
    public function updateForChangePasswordUsingMIDWithSP($MID, $changePassword) {
        ...
        try {
            $sql = "CALL sp_update_data(1, 0, 'MID', 370, 'ForChangePassword', '1', @OUT_intResultCode, @OUT_strResult)";
            $param = array(':ChangePassword' => $changePassword,':MID' => $MID);
            $command = $this->_connection->createCommand($sql);
            $command->bindValues($param);
            $command->execute();
    ...
    public function getMemberDetailsByMIDWithSP($MID) {
        $sql = "CALL sp_select_data(1,0,0,'$MID','UserName',@OUTRetCode,@OUTRetMessage,@OUTfldListRet)";
        $param = array(':MID' => $MID);
        $command = $this->_connection->createCommand($sql);
        $result = $command->queryRow(true,$param);
        $result = explode(";", $result['OUTfldListRet']);
        
        return array('UserName' => $result[0]); //'Status' => $result[1], 'Password' => $result[2]);//, 'MID' => $MID);
    }
    ...
    {
        $sql = 'SELECT IsVIP FROM members WHERE MID = :MID'; //AND Status = 9';
        $param = array(':MID' => $MID);
        $command = $this->_connection->createCommand($sql);
        $result = $command->queryRow(true, $param);
