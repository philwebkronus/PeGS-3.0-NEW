...
Class RedemptionProcess extends BaseEntity {
    ...
    public function ProcessItemRedemption($MID, $RewardItemID, $Quantity, $RedeemedPoints, $CardNumber, $Source, $RedeemedDate){
        ...
        $siteid = $_SESSION['userinfo']['SiteID'];
                ...
                if($LastInsertedID != "" && !$_ItemRedemptionLogs->HasError)
                {
                    ...
                    { 
                        ...
                        if($OldCP >= $totalpoints)
                        {
                            ...
                            if(App::getParam('PointSystem') == 2 ) {
                                $isPointsDeducted = $_PcwsWrapper->deductCompPoints($CardNumber, $totalpoints, $siteid, 1);
                                //$PlayerPoints = $_MemberCards->getCurrentPointsByCardNumber($CardNumber);
                                $OldCP = $_PcwsWrapper->getCompPoints($CardNumber, 1);
                                $OldCP = $OldCP['GetCompPoints']['CompBalance'];
                            }
...
//                            $OldCP = $PlayerPoints[0]['CurrentPoints'];
                            ...
                            if($OldCP >= $RedeemedPoints){
                                ...
                                if(App::getParam('PointSystem') == 2) {
                                    $isPointsDeducted = $_PcwsWrapper->deductCompPoints($CardNumber, $RedeemedPoints, $siteid, 1);
