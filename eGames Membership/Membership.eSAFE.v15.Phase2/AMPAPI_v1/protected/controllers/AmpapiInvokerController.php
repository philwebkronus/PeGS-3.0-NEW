...
//
    ...
    public function actionAutoRegisterMember() {
        $this->pageTitle = $this->genTitlePage('Auto Register Member');
        $result = '';
...
var_dump($_POST);exit;
        if (isset($_POST['TPSessionID']) || isset($_POST['FirstName']) || isset($_POST['LastName']) || isset($_POST['Password']) || isset($_POST['PermanentAdd']) || isset($_POST['MobileNo']) || isset($_POST['EmailAddress']) && isset($_POST['IDPresented']) && isset($_POST['IDNumber']) && isset($_POST['Birthdate'])) {
            ...
            $result = $this->_autoregisterMember($TPSessionID, $FirstName, $MiddleName, $LastName, $NickName, $Password, $PermanentAdd, $MobileNo, $AlternateMobileNo, $EmailAddress, $AlternateEmail, $Gender, $IDPresented, $IDNumber, $Nationality, $Birthdate, $Occupation, $IsSmoker, $ReferralCode, $ReferrerID, $EmailSubscription, $SMSubscription, $moduleName, $CivilStatus, $RegisterFor, $UBCard, $AID, $SiteID);
        }
        $this->render($moduleName, array('result' => $result));
    }

    private function _autoregisterMember($TPSessionID, $FirstName, $MiddleName, $LastName, $NickName, $Password, $PermanentAdd, $MobileNo, $AlternateMobileNo, $EmailAddress, $AlternateEmail, $Gender, $IDPresented, $IDNumber, $Nationality, $Birthdate, $Occupation, $IsSmoker, $ReferralCode, $ReferrerID, $EmailSubscription, $SMSubscription, $moduleName, $CivilStatus, $RegisterFor, $UBCard, $AID, $SiteID) {
        $url = $this->genURL($moduleName);
        $postData = CJSON::encode(array('TPSessionID' => $TPSessionID, 'FirstName' => $FirstName, 'MiddleName' => $MiddleName, 'LastName' => $LastName,
                    'NickName' => $NickName, 'Password' => $Password, 'PermanentAdd' => $PermanentAdd, 'MobileNo' => $MobileNo, 'AlternateMobileNo' => $AlternateMobileNo,
                    'EmailAddress' => $EmailAddress, 'AlternateEmail' => $AlternateEmail, 'Gender' => $Gender, 'IDPresented' => $IDPresented, 'IDNumber' => $IDNumber,
                    'Nationality' => $Nationality, 'Birthdate' => $Birthdate, 'Occupation' => $Occupation, 'IsSmoker' => $IsSmoker, 'ReferralCode' => $ReferralCode,
                    'ReferrerID' => $ReferrerID, 'EmailSubscription' => $EmailSubscription, 'SMSSubscription' => $SMSubscription,
                    'CivilStatus' => $CivilStatus, 'RegisterFor' => $RegisterFor, 'UBCard' => $UBCard, 'AID' => $AID, 'SiteID' => $SiteID
        ));
        $result = $this->SubmitData($url, $postData);
    ...
    public function actionResetPin() {
        $this->pageTitle = $this->genTitlePage('Reset PIN');
        $result = '';
        ...
        if (isset($_POST['txtcardnumber'])) {
            ...
            $result = $this->_resetPin($txtcardnumber, $moduleName);
        }

        $this->render('resetpin', array('result' => $result));
    }

    private function _resetPin($CardNumber, $moduleName) {
        $url = $this->genURL($moduleName);
    ...
    public function actionRedeemCompPoints() {
        $this->pageTitle = $this->genTitlePage('Redeem CompPoints');

        $result = '';
        $moduleName = 'redeemcomppoints';

        if (isset($_POST['TPSessionID']) || isset($_POST['MPSessionID']) || isset($_POST['CardNumber']) || isset($_POST['Quantity']) || isset($_POST['Source'])) {
            $TPSessionID = $_POST['TPSessionID'];
            $MPSessionID = $_POST['MPSessionID'];
            $CardNumber = $_POST['CardNumber'];
            $Quantity = $_POST['Quantity'];
            $Source = $_POST['Source'];
            //$config = Yii::app()->params['config'];
            $result = $this->_redeemCompPoints($TPSessionID, $MPSessionID, $CardNumber, $Quantity, $Source, $moduleName);
        }

        $this->render($moduleName, array('result' => $result));
    }

    private function _redeemCompPoints($TPSessionID, $MPSessionID, $CardNumber, $Quantity, $Source, $moduleName) {
        $url = $this->genURL($moduleName);
        $postData = CJSON::encode(array('TPSessionID' => $TPSessionID, 'MPSessionID' => $MPSessionID, 'CardNumber' => $CardNumber, 'Quantity' => $Quantity, 'Source' => $Source));
        $result = $this->SubmitData($url, $postData);
    ...
    public function actionGetCivilStatus() {
        $this->pageTitle = $this->genTitlePage('Get CivilStatus');
        ...
        if (isset($_POST['TPSessionID'])) {
            $TPSessionID = $_POST['TPSessionID'];
            $result = $this->_getCivilStatus($TPSessionID, $moduleName);
        }

        $this->render($moduleName, array('result' => $result));
    }

    private function _getCivilStatus($TPSessionID, $moduleName) {
        $url = $this->genURL($moduleName);
        $postData = CJSON::encode(array('TPSessionID' => $TPSessionID));
        $result = $this->SubmitData($url, $postData);
    ...
    public function actionGetRegisterFor() {
        $this->pageTitle = $this->genTitlePage('Get RegisterFor');
        ...
        if (isset($_POST['TPSessionID'])) {
            $TPSessionID = $_POST['TPSessionID'];
            $result = $this->_getRegisterFor($TPSessionID, $moduleName);
        }

        $this->render($moduleName, array('result' => $result));
    }

    private function _getRegisterFor($TPSessionID, $moduleName) {
        $url = $this->genURL($moduleName);
        $postData = CJSON::encode(array('TPSessionID' => $TPSessionID));
        $result = $this->SubmitData($url, $postData);
