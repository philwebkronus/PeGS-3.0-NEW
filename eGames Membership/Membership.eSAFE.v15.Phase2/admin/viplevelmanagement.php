...
$casinoservice->ShowCaption = false;
...
$hdnCasinoService = new Hidden("hdnCasinoService", "hdnCasinoService");
...
$fproc->AddControl($casinoservice);
$fproc->AddControl($classification_type);
$fproc->AddControl($hdnProcess);
$fproc->AddControl($hdnCasinoService);
$fproc->ProcessForms();
...
//            var_dump($serviceid);exit;
            ...
            if(!empty($abbottmemservice))
            {
                ...
                if(!empty($pid))
                {
                    ...
                    $getVip = $casinoAPI->GetPlayerClassification('RTG2', $pid, $serverID);
                    header("Content-Type:text/html");
                    if($getVip['IsSucceed'] == true) {
                        $currentVip = $getVip['ClassID'];
                        // If success add to VIP Level
                        if($vipLevelID == $currentVip)
                        {
                            //Validate input if empty
                            if ($vipLevelID != "" && $description != "")
                            {
                                //Check if record exist
                                $isExisting = $vip_levels->checkIfExist($vipLevelID, $description, $serviceid);
                                if(count($isExisting) == 0)
                                {
                                    $isExisting = $vip_levels->checkIfVipNameExist($description, $serviceid);
                                    if(count($isExisting) == 0) {
                                        $result = $vip_levels->addVipLevel($vipLevelID, $description, $serviceid);
                                    }
                                    else
                                    {
                                        $showdialog = true;
    ...
    elseif($process == 2)
    {
        $serviceid = $_POST['hdnCasinoService'];
...
//            var_dump($serviceid);exit;
        ...
        $isVipLevelExist = $memberServices->checkVIPLevel($vipLevelID, $serviceid);

        if(count($isVipLevelExist) == 0)
        {
            $result = $vip_levels->updateVipStatus($vipLevelID, $description, $serviceid, $status);
        ...
        {
            $showdialog = true;
            $msg = "Updating VIP Level Status Failed, VIP level is being used.";
            $title = "ERROR MESSAGE";
        }
    ...
    $(document).ready(function() {
        ...
        function getVipLevels()
        {
            ...
            jQuery("#viplevels").jqGrid({
                ...
                datatype: "json",
                mtype: "POST",
                colNames: ['VIPLevelID', 'Name', 'ServiceName', 'Status', 'Action' ],
                colModel: [
                    {name: 'VIPLevelID', index: 'VIPLevelID', align: 'left'},
                    {name: 'Name', index: 'Name', align: 'left'},
                    {name: 'ServiceName', index: 'ServiceID', align: 'left'},
        ...
        $("#editvip").live('click', function(e){
            e.preventDefault();
            var casinoservice = $(this).attr('serviceid');
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('#submitAddVip').live('click', function(e){
            ...
            if($('#casinoservice').val() == '-1' || $('#txtVipLevelID').val() == '' || $('#txtVipLevelID').val() == ''){
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#hdnprocess").val(1);
        $("#casinoservice").prop("disabled",false);
...
<form action ="" id="submitaddviplevel" method="post" >
    <div id="addviplevel">
        <table id="tblviplevel" style="margin-top: 50px;">
            <tr>
                <td>Casino Service: </td>
                <td><?php echo $casinoservice; ?></td>
            </tr>
            <tr>
                <td>VIP Level ID: </td>
            ...
            <tr id="hiddenstatus" name="hiddenstatus" style="display : none">
                ...
                <?php echo $hdnName; echo $hdnVipLevel; echo $hdnCasinoService; ?>
            </tr>
        </table>
