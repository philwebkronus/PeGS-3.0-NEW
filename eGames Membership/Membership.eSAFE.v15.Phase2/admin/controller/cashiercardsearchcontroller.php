...
if ($fproc->IsPostBack)
{
    if ($btnSearch->SubmittedValue == "Search" && $txtSearch->SubmittedValue != "")
    {
        ...
        {
            //Search by email or username, not allowed
        ...
        {
            // Only UB cards, where corresponding temp card was in the list of PW pre-generated cards are allowed to be searched here
            $membercards = $_MemberCards->getMemberCardInfoByCard($searchValue);
            if (count($membercards) > 0)
            {
                // Get corresponding temporary card
                $CardNumber = "";
                $tempCode = $_MemberCards->getTempCardByMID($membercards[0]['MID']);
                if(isset($tempCode[0]) && $tempCode[0] != "")
                {
                    if ($membercards[0]['CardTypeID'] != 3)
                    {    
                        $CardNumber = $tempCode[0]['CardNumber'];
                        $result = $_MembersTemp->checkifpregeneratedtemp($CardNumber);
                        if(isset($result[0]) && $result[0] != "")
                        {
                            $_SESSION['CardInfo']['IsCashierEdited'] = $result[0]['IsEdited'];                                                                    
                            $_SESSION['CardInfo']['TempCode'] = $result[0]['TemporaryAccountCode'];                                                                    
                        ...
                        {
                            App::SetErrorMessage('You are not allowed to search or view this card number.');
                            $error = "You are not allowed to search or view this card number.";
                            $logger->logger($logdate, $logtype, $error); 
                        }
                    }
                    else
