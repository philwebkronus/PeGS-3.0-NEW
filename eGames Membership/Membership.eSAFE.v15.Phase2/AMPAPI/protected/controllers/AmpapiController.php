...
class AmpapiController extends Controller {
    ...
    private $ApiMethodID = array(
        ...
        'ListPromos' => APILogsModel::API_LIST_PROMOS,
        'ListLocations' => APILogsModel::API_LIST_LOCATIONS
    );
    ...
    public function actionListPromos() {
        ...
        $validateRequiredField = $this->validateRequiredFields($request, $module, array('TPSessionID' => false, 'MPSessionID' => false, 'ViewBy' => false), $rand);
        if ($validateRequiredField === true) {
            $TPSessionID = trim($request['TPSessionID']);
            $validateTPSessionID = $this->_validateTPSession($TPSessionID, 'GetActiveSession', $module, $rand);
            if ($validateTPSessionID === true) {
                ...
                $moduleName = strtolower($module);
                $url = $this->genMPAPIURL($moduleName);
                $postData = CJSON::encode(array('MPSessionID'=>$MPSessionID,'ViewBy'=>$viewBy));
                $result = $this->SubmitData($url, $postData);
                $AID = $this->currentAID;
                ...
                if (isset($result[0]) && $result[0] == 200) {
                    ...
                    $ValidateResponse = $this->validateResponse($result[1], $module);
                    if($ValidateResponse==true){
                        $this->_auditTrail(AuditTrailModel::LIST_PROMOS,0,$AID, $TPSessionID, $module, $viewBy);
                        $this->_apiLogs(APILogsModel::API_LIST_PROMOS,'' , 0, '', 1, $module);
                    }
                } else {
                    $this->_displayCustomMessages(73, $module, 'No response from Membership portal API.', $rand);
                    $this->_apiLogs(APILogsModel::API_LIST_PROMOS, '', 73, '', 2, $module);
                }
            }
        }
    }
        ...
        if ($validateRequiredField === true) {
            ...
            if ($validateTPSessionID === true) {
                ...
                if (isset($result[0]) && $result[0] == 200) {
                    ...
                    $ValidateResponse = $this->validateResponse($result[1], $module);
                    if($ValidateResponse==true){
                        $this->_auditTrail(AuditTrailModel::LIST_LOCATIONS,0,$AID, $TPSessionID, $module, '');
                        $this->_apiLogs(APILogsModel::API_LIST_LOCATIONS,'' , 0, '', 1, $module);
                    }
                } else {
                    $this->_displayCustomMessages(73, $module, 'No response from Membership portal API.', $rand);
                    $this->_apiLogs(APILogsModel::API_LIST_LOCATIONS, '', 73, '', 2, $module);
                }
            }
        }
    }
