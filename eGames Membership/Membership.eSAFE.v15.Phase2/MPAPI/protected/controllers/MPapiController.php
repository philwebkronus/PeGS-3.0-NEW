...
/**
 * Controller for Membership Portal API v2
 * @date 6-13-2014
 * @author fdlsison
 */
class MPapiController extends Controller {
    ...
    private function _getResultForAuthenticate($apiMethod, $errorCode, $transMsg)
    {
        if($apiMethod == 1)
        {
            $module = 'Login';
        ...
        $result = array('data' => $data, 'message' => $message);

        return $result;
    }
    //@date 07-10-2014
    //@purpose Login authentication
    private function _authenticate($username, $password, $apiMethod) {
        if($apiMethod == 1)
            $module = 'LOGIN';
        else
            $module = 'CREATEMOBILEINFO';
        ...
        if (Utilities::validateEmail($username)) {
            ...
            if(!$result)
            {
                ...
                //$data = CommonController::retMsgLogin($module, '', '', '', '', $errorCode, $transMsg, '');
                $authResult = $this->_getResultForAuthenticate($apiMethod, $errorCode, $transMsg);
                ...
                $logger->log($logger->logdate, "[".$module." ERROR]: " . $username . " || ", $logMessage);
                $apiDetails = $module."-Authenticate-Failed: Member not found.";
...
//                $message = "[Login] Output: " . CJSON::encode($data);
                ...
                $logger->log($logger->logdate, "[".$module." ERROR]: " . $username . " || ", $logMessage);
                $apiDetails = $module.'-Authenticate-Failed: Card number does not exist.';
...
//            }
        ...
        if (is_array($result) && count($result) > 0) {
            ...
            switch ($result['Status']) {
                case 1:
                    if ($result['Password'] != $strPass) {
                        ...
                        $logger->log($logger->logdate, "[".$module." ERROR]: " . $username . " || ", $logMessage);
                        $apiDetails = $module.'-Authenticate-Failed: Incorrect password.';
                        $isInserted = $apiLogsModel->insertAPIlogs($apiMethod, $refID, $apiDetails, '', 2);
                        if ($isInserted == 0) {
                            $logMessage = "Failed to insert to APILogs.";
                            $logger->log($logger->logdate, "[".$module." ERROR]: " . $username . " || ", $logMessage);
                ...
                case 0:
                    ...
                    $logger->log($logger->logdate, "[".$module." ERROR]: " . $username . " || ", $logMessage);
                    $apiDetails = $module.'-Authenticate-Failed: Member account is inactive.';
                    $isInserted = $apiLogsModel->insertAPIlogs($apiMethod, $refID, $apiDetails, '', 2);
                    if ($isInserted == 0) {
                        $logMessage = "Failed to insert to APILogs.";
                        $logger->log($logger->logdate, "[".$module." ERROR]: " . $username . " || ", $logMessage);
                ...
                case 2:
                    ...
                    $logger->log($logger->logdate, "[".$module." ERROR]: " . $username . " || ", $logMessage);
                    $apiDetails = $module.'-Authenticate-Failed: Member account is suspended.';
                    $isInserted = $apiLogsModel->insertAPIlogs($apiMethod, $refID, $apiDetails, '', 2);
                    if ($isInserted == 0) {
                        $logMessage = "Failed to insert to APILogs.";
                        $logger->log($logger->logdate, "[".$module." ERROR]: " . $username . " || ", $logMessage);
                ...
                case 3:
                    ...
                    $logger->log($logger->logdate, "[".$module." ERROR]: " . $username . " || ", $logMessage);
                    $apiDetails = $module.'-Authenticate-Failed: Member account is locked (login attempts).';
                    $isInserted = $apiLogsModel->insertAPIlogs($apiMethod, $refID, $apiDetails, '', 2);
                    if ($isInserted == 0) {
                        $logMessage = "Failed to insert to APILogs.";
                        $logger->log($logger->logdate, "[".$module." ERROR]: " . $username . " || ", $logMessage);
                ...
                case 4:
                    ...
                    $logger->log($logger->logdate, "[".$module." ERROR]: " . $username . " || ", $logMessage);
                    $apiDetails = $module.'-Authenticate-Failed: Member account is locked (admin).';
                    $isInserted = $apiLogsModel->insertAPIlogs($apiMethod, $refID, $apiDetails, '', 2);
                    if ($isInserted == 0) {
                        $logMessage = "Failed to insert to APILogs.";
                        $logger->log($logger->logdate, "[".$module." ERROR]: " . $username . " || ", $logMessage);
                ...
                case 6:
                    ...
                    $logger->log($logger->logdate, "[".$module." ERROR]: " . $username . " || ", $logMessage);
                    $apiDetails = $module.'-Authenticate-Failed: Member account is terminated.';
                    $isInserted = $apiLogsModel->insertAPIlogs($apiMethod, $refID, $apiDetails, '', 2);
                    if ($isInserted == 0) {
                        $logMessage = "Failed to insert to APILogs.";
                        $logger->log($logger->logdate, "[".$module." ERROR]: " . $username . " || ", $logMessage);
                ...
                default:
                    ...
                    $logger->log($logger->logdate, "[".$module." ERROR]: " . $username . " || ", $logMessage);
                    $apiDetails = $module.'-Authenticate-Failed: Member account is invalid.';
                    $isInserted = $apiLogsModel->insertAPIlogs($apiMethod, $refID, $apiDetails, '', 2);
                    if ($isInserted == 0) {
                        $logMessage = "Failed to insert to APILogs.";
                        $logger->log($logger->logdate, "[".$module." ERROR]: " . $username . " || ", $logMessage);
            ...
            $logger->log($logger->logdate, "[".$module." ERROR]: " . $username . " || ", $logMessage);
            $apiDetails = $module.'-Authenticate-Failed: Member card is banned.';
            $isInserted = $apiLogsModel->insertAPIlogs($apiMethod, $refID, $apiDetails, '', 2);
            if ($isInserted == 0) {
                $logMessage = "Failed to insert to APILogs.";
                $logger->log($logger->logdate, "[".$module." ERROR]: " . $username . " || ", $logMessage);
           ...
           // if ($isTempAcctExist > 0) {
                ...
                $logger->log($logger->logdate, "[".$module." ERROR]: " . $username . " || ", $logMessage);
                $apiDetails = $module.'-Authenticate-Failed: You need to transact at least one transaction before you can login.';
                $isInserted = $apiLogsModel->insertAPIlogs($apiMethod, $refID, $apiDetails, '', 2);
                if ($isInserted == 0) {
                    $logMessage = "Failed to insert to APILogs.";
                    $logger->log($logger->logdate, "[".$module." ERROR]: " . $username . " || ", $logMessage);
    ...
    public function actionLogin() {
        ...
        if (isset($request['Username']) && isset($request['Password']) && isset($request['AlterStr'])) {
            if (($request['Username'] == '') || ($request['Password'] == '') || ($request['AlterStr'] == '')) {
                ...
                $appLogger->log($appLogger->logdate, "[response]", $message);
                $this->_sendResponse(200, CJSON::encode($data));
                $logger->log($logger->logdate, "[LOGIN ERROR]: " . $request['Username'] . " || " . $request['AlterStr'] . " || ", $logMessage);
                ...
                if ((Utilities::validateEmail($request['Username']) || ctype_alnum($request['Username'])) && ctype_alnum($request['Password']) && is_numeric($request['AlterStr'])) {
                    ...
                    $members = $this->_authenticate($username, $password, $apiMethod);

                    if ($members) {
                        $result = $memberCardsModel->getMemberStatus($username);

//                           var_dump($transMsg);exit;
//                         check if status is not active
                        ...
                        if ($result > 0) {
                            ...
                            if ($isUpdated > 0) {
                                ...
                                $errorCode = 0;
                                $data = CommonController::retMsgLogin($module, $mpSessionID, $cardTypeID, $isVIP, $cardNumber, $errorCode, $transMsg, $remarks);
                                $message = "[Login] Output: " . CJSON::encode($data);
                                $appLogger->log($appLogger->logdate, "[response]", $message);
                                $this->_sendResponse(200, CJSON::encode($data));
                ...
                if (Utilities::validateEmail($emailCardNumber)) {
                    ...
                    if ($data && count($isCardExist) > 0) {
                        if (($data['Status'] == 1) || ($data['Status'] == 5)) {
                            ...
                            $this->_sendResponse(200, CJSON::encode($data));
                            $logger->log($logger->logdate, "[FORGOTPASSWORD ERROR]: " . $emailCardNumber . " || ", $logMessage);
                            $apiDetails = 'FORGOTPASSWORD-Failed: ' . $transMsg . '.' . 'Status = ' . $status . ' MID = ' . $MID;
                            $isInserted = $apiLogsModel->insertAPIlogs($apiMethod, $refID, $apiDetails, '', 2);
                            if ($isInserted == 0) {
                                $logMessage = "Failed to insert to APILogs.";
                                $logger->log($logger->logdate, "[FORGOTPASSWORD ERROR]: " . $emailCardNumber . " || ", $logMessage);
    ...
    public function actionCheckPoints() {
        ...
        if (isset($request['CardNumber'])) {
            if ($request['CardNumber'] == '') {
                ...
                $appLogger->log($appLogger->logdate, "[response]", $message);
                $this->_sendResponse(200, CJSON::encode($data));
                $logMessage = "One or more fields is not set or is blank. CardNumber is blank.";
...
//                            else
//                                $MID = 0;
                            if($currentPoints == "" || $currentPoints == null) {
                                $currentPoints = 0;
                            }
                            if (!empty($memberDetails)) {
...
//                            $MID = 0;
        ...
        if (isset($result)) {
            ...
            if (isset($request['CardNumber']) && isset($request['RewardItemID']) && isset($request['Quantity'])
                ...
                if (($request['CardNumber'] == '') || ($request['RewardItemID'] == '') || ($request['RewardID'] == '')
                    ...
                    if (($request['Tracking1'] == '')) {
                        $transMsg = $transMsg . "[Tracking1] ";
                    }
                    if (($request['Tracking2'] == '')) {
                        $transMsg = $transMsg . "[Tracking2] ";
                    }
    ...
    //                                                        }
                                                            ...
                                                            $importantReminder = Yii::app()->params['rewarditem_imagepath'] . "important_reminders.jpg";
    ...
    $itemRedemptionArray = array('ItemImage' => $itemImage, 'ItemName' => $itemName, 'PartnerName' => $partnerName, 'PlayerName' => $playerName, 'CardNumber' => $cardNumber, 'RedemptionDate' => $redemptionDate, 'SerialNumber' => $sessionSerialCode[$i], 'SecurityCode' => $sessionSecurityCode[$i], 'ValidityDate' => $sessionValidUntil, 'CompanyAddress' => $companyAddress, 'CompanyPhone' => $companyPhone, 'CompanyWebsite' => $companyWebsite, 'Quantity' => $quantity, 'SiteCode' => $siteCode, 'PromoCode' => $promoCode, 'PromoTitle' => $promoTitle,
    'PromoPeriod' => $promoPeriod, 'DrawDate' => $drawDate, 'Address' => $address, 'Birthdate' => $birthdate,  'EmailAddress' => $email, 'ContactNo' => $contactNo, 'CheckSum' => $checkSum, 'About' => $about, 'Terms' => $terms);
                                                            }
                                                            Utilities::log("ReturnMessage: " . $transMsg . " ErrorCode: " . $errorCode);
                ...
                if ($isExist['COUNT(*)'] > 0) {
                    ...
                    if ($MID == '' || $MID == null) {
                        ...
                        if ($result) {
                            ...
                            if($memberPoints['RedeemedPoints'] == null)
                                $redeemedPoints = 0;
                            else
                                $redeemedPoints = $memberPoints['RedeemedPoints'];

                            if($memberPoints['LifetimePoints'] == null)
                                $lifetimePoints = 0;
                            else
    ...
    //@purpose get promos list
    public function actionListPromos() {
        $request = $this->_readJsonRequest();
        ...
        $apiLogsModel = new APILogsModel();
        $memberSessionsModel = new MemberSessionsModel();
        $mobileAppAPILogsModel = new MobileAppAPILogsModel();

        $result = $memberSessionsModel->getMID($request['MPSessionID']);
        $MID = $result['MID'];

        $promosList = array('SiteName' => $siteName, 'PromoName' => $promoName, 'PromoDetails' => $promoDetails, 'StartDate' => $startDate, 'EndDate' => $endDate, 'DrawDate' => $drawDate, 'Status' => $status, 'PromoThumbnail' => $promoThumbnail, 'PromoPoster' => $promoPoster);
        $isValid = $this->_validateMPSession($request['MPSessionID']);
        if (isset($isValid) && !$isValid) {
            $transMsg = "MPSessionID does not exist.";
            $errorCode = 13;
            Utilities::log("ReturnMessage: " . $transMsg . " ErrorCode: " . $errorCode);
            $data = CommonController::retMsgListPromos($module, $promosList, $errorCode, $transMsg);
            $message = "[" . $module . "] Output: " . CJSON::encode($data);
            $appLogger->log($appLogger->logdate, "[response]", $message);
            $this->_sendResponse(200, CJSON::encode($data));
            $logMessage = 'MPSessionID does not exist.';
            $logger->log($logger->logdate, "[LISTPROMOS ERROR]: MID " . $MID . " || ", $logMessage);
            $apiDetails = 'LISTPROMOS-Failed: MPSessionID does not exist.. MID = ' . $MID;
            $isInserted = $apiLogsModel->insertAPIlogs($apiMethod, '', $apiDetails, '', 2);
            if ($isInserted == 0) {
                $logMessage = "Failed to insert to APILogs.";
                $logger->log($logger->logdate, "[LISTPROMOS ERROR]: MID " . $MID . " || ", $logMessage);
            }
            $isInserted = $mobileAppAPILogsModel->insertAPIlogs($mobileApiMethod, '', $apiDetails, '', 2);
            if ($isInserted == 0) {
                $logMessage = "Failed to insert to Mobile App APILogs.";
                $logger->log($logger->logdate, "[LISTPROMOS ERROR]: MID " . $MID . " || ", $logMessage);
            }
            exit;
        }
        ...
        if (isset($result)) {
            ...
            if ($isUpdated == 0) {
                ...
                $this->_sendResponse(200, CJSON::encode($data));
                $logger->log($logger->logdate, "[LISTPROMOS ERROR]: MID " . $MID . " || ", $logMessage);
                $apiDetails = 'LISTPROMOS-UpdateTransDate-Failed: ' . 'MID = ' . $MID . ' SessionID = ' . $request['MPSessionID'];
                $isInserted = $apiLogsModel->insertAPIlogs($apiMethod, '', $apiDetails, '', 2);
                if ($isInserted == 0) {
                    $logMessage = "Failed to insert to APILogs.";
                    $logger->log($logger->logdate, "[LISTPROMOS ERROR]: MID " . $MID . " || ", $logMessage);
        ...
        if (isset($request['MPSessionID']) && isset($request['ViewBy'])) {
            if ($request['MPSessionID'] == '' || $request['ViewBy'] == '') {
                $transMsg = "One or more fields is not set or is blank.";
                $errorCode = 1;
                Utilities::log("ReturnMessage: " . $transMsg . " ErrorCode: " . $errorCode);
                $data = CommonController::retMsgListPromos($module, $promosList, $errorCode, $transMsg);
                ...
                $logMessage = 'One or more fields is not set or is blank.';
                $logger->log($logger->logdate, "[LISTPROMOS ERROR]: MID " . $MID . " || ", $logMessage);
                $apiDetails = 'LISTPROMOS-Failed: Invalid input parameters.';
                $isInserted = $apiLogsModel->insertAPIlogs($apiMethod, '', $apiDetails, '', 2);
                if ($isInserted == 0) {
                    $logMessage = "Failed to insert to APILogs.";
                    $logger->log($logger->logdate, "[LISTPROMOS ERROR]: MID " . $MID . " || ", $logMessage);
                ...
                $viewBy = trim($request['ViewBy']);
                $mpSessionID = trim($request['MPSessionID']);

                $auditTrailModel = new AuditTrailModel();
                $tblPromoMainModel = new TblPromoMainModel();

                if ($viewBy != 1 && $viewBy != 2) {
                    $transMsg = "Please input 1 for online or 2 for on-site.";
                    $errorCode = 98;
                    Utilities::log("ReturnMessage: " . $transMsg . " ErrorCode: " . $errorCode);
                    $data = CommonController::retMsgListPromos($module, $promosList, $errorCode, $transMsg);
                    $message = "[" . $module . "] Output: " . CJSON::encode($data);
                    $appLogger->log($appLogger->logdate, "[response]", $message);
                    $this->_sendResponse(200, CJSON::encode($data));
                    $logMessage = 'Please input 1 for online or 2 for on-site.';
                    $logger->log($logger->logdate, "[LISTPROMOS ERROR]: MID " . $MID . " || ", $logMessage);
                    $apiDetails = 'LISTPROMOS-Failed: Invalid input parameter(s)';
                    $isInserted = $apiLogsModel->insertAPIlogs($apiMethod, '', $apiDetails, '', 2);
                    if ($isInserted == 0) {
                        $logMessage = "Failed to insert to APILogs.";
                        $logger->log($logger->logdate, "[LISTPROMOS ERROR]: MID " . $MID . " || ", $logMessage);
                ...
                if ($memberSessions)
                    ...
                    $logMessage = 'MPSessionID does not exist.';
                    $logger->log($logger->logdate, "[LISTPROMOS ERROR]: MID " . $MID . " || ", $logMessage);
                    $apiDetails = 'LISTPROMOS-Failed: There is no active session. MID = ' . $MID;
                    $isInserted = $apiLogsModel->insertAPIlogs($apiMethod, '', $apiDetails, '', 2);
                    if ($isInserted == 0) {
                        $logMessage = "Failed to insert to APILogs.";
                        $logger->log($logger->logdate, "[LISTPROMOS ERROR]: MID " . $MID . " || ", $logMessage);
                ...
                if (count($isExist) > 0) {
                    ...
                    if($viewBy == 2)
                        $promoOffers = $tblPromoMainModel->getSitePromos($viewBy);
                    else
                        $promoOffers = $tblPromoMainModel->getAllPromosBasedOnViewBy($viewBy);

                    for ($itr = 0; $itr < count($promoOffers); $itr++) {
                        $promoName[$itr] = $promoOffers[$itr]['fldPromoName'];
                        $promoDetails[$itr] = $promoOffers[$itr]['fldPromoDetails'];
                    ...
                    $isSuccessful = $auditTrailModel->logEvent(AuditTrailModel::API_LIST_PROMOS, 'ViewBy: ' . $viewBy, array('MID' => $MID, 'SessionID' => $mpSessionID));
                    ...
                    $transMsg = 'No Error, Transaction successful.';
                    $errorCode = 0;
                    $data = CommonController::retMsgListPromos($module, $promos, $errorCode, $transMsg);
                    $message = "[" . $module . "] Output: " . CJSON::encode($data);
                    $appLogger->log($appLogger->logdate, "[response]", $message);
                    $this->_sendResponse(200, CJSON::encode($data));
                    //$this->_sendResponse(200, CJSON::encode(CommonController::retMsgListItems($module, $items, $errorCode, $transMsg)));
                    $logMessage = 'List Promos is successful.';
                    $logger->log($logger->logdate, "[LISTPROMOS SUCCESSFUL]: MID " . $MID . " || ", $logMessage);
                    $apiDetails = 'LISTPROMOS-Successful: MID = ' . $MID;
                    $isInserted = $apiLogsModel->insertAPIlogs($apiMethod, $refID, $apiDetails, '', 1);
                    if ($isInserted == 0) {
                        $logMessage = "Failed to insert to APILogs.";
                        $logger->log($logger->logdate, "[LISTPROMOS ERROR]: MID " . $MID . " || ", $logMessage);
                    }
                    $isInserted = $mobileAppAPILogsModel->insertAPIlogs($mobileApiMethod, $refID, $apiDetails, '', 1);
                    if ($isInserted == 0) {
                        $logMessage = "Failed to insert to Mobile App APILogs.";
                        $logger->log($logger->logdate, "[LISTPROMOS ERROR]: MID " . $MID . " || ", $logMessage);
                    }
                    ...
                    $isInserted = $mobileAppAPILogsModel->insertAPIlogs($mobileApiMethod, $refID, $apiDetails, '', 2);
                    if ($isInserted == 0) {
                        $logMessage = "Failed to insert to Mobile App APILogs.";
                        $logger->log($logger->logdate, "[LISTPROMOS ERROR]: MID " . $MID . " || ", $logMessage);
                    }
    ...
    public function actionListLocations() {
        ...
        if (isset($isValid) && !$isValid) {
            ...
            $errorCode = 13;
            Utilities::log("ReturnMessage: " . $transMsg . " ErrorCode: " . $errorCode);
            $data = CommonController::retMsgListLocations($module, $locationsList, $errorCode, $transMsg);
            $message = "[" . $module . "] Output: " . CJSON::encode($data);
            $appLogger->log($appLogger->logdate, "[response]", $message);
            $this->_sendResponse(200, CJSON::encode($data));
            $logMessage = 'MPSessionID does not exist.';
            $logger->log($logger->logdate, "[LISTLOCATIONS ERROR]: MID " . $MID . " || ", $logMessage);
            $apiDetails = 'LISTLOCATIONS-Failed: MPSessionID does not exist.. MID = ' . $MID;
            $isInserted = $apiLogsModel->insertAPIlogs($apiMethod, '', $apiDetails, '', 2);
            if ($isInserted == 0) {
                $logMessage = "Failed to insert to APILogs.";
                $logger->log($logger->logdate, "[LISTLOCATIONS ERROR]: MID " . $MID . " || ", $logMessage);
        ...
        if (isset($result)) {
            ...
            if ($isUpdated == 0) {
                ...
                $this->_sendResponse(200, CJSON::encode($data));
                $logger->log($logger->logdate, "[LISTLOCATIONS ERROR]: MID " . $MID . " || ", $logMessage);
                $apiDetails = 'LISTLOCATIONS-UpdateTransDate-Failed: ' . 'MID = ' . $MID . ' SessionID = ' . $request['MPSessionID'];
                $isInserted = $apiLogsModel->insertAPIlogs($apiMethod, '', $apiDetails, '', 2);
                if ($isInserted == 0) {
                    $logMessage = "Failed to insert to APILogs.";
                    $logger->log($logger->logdate, "[LISTLOCATIONS ERROR]: MID " . $MID . " || ", $logMessage);
        ...
        if (isset($request['MPSessionID'])) {
            if ($request['MPSessionID'] == '') {
                ...
                $logMessage = 'One or more fields is not set or is blank.';
                $logger->log($logger->logdate, "[LISTLOCATIONS ERROR]: MID " . $MID . " || ", $logMessage);
                $apiDetails = 'LISTLOCATIONS-Failed: Invalid input parameters.';
                $isInserted = $apiLogsModel->insertAPIlogs($apiMethod, '', $apiDetails, '', 2);
                if ($isInserted == 0) {
                    $logMessage = "Failed to insert to APILogs.";
                    $logger->log($logger->logdate, "[LISTLOCATIONS ERROR]: MID " . $MID . " || ", $logMessage);
                ...
                $auditTrailModel = new AuditTrailModel();
                $tblPegsSiteInfoModel = new TblPegsSiteInfoModel();
                $tblNonPegsSiteInfoModel = new TblNonPegsSiteInfoModel();
                $ref_IslandsModel = new Ref_IslandsModel();
                $ref_RegionsModel = new Ref_RegionsModel();
                $ref_ProvincesModel = new Ref_ProvincesModel();
                $ref_CitiesModel = new Ref_CitiesModel();
                $ref_BarangayModel = new Ref_BarangayModel();
                ...
                if ($memberSessions)
                    ...
                    $logMessage = 'MPSessionID does not exist.';
                    $logger->log($logger->logdate, "[LISTLOCATIONS ERROR]: MID " . $MID . " || ", $logMessage);
                    $apiDetails = 'LISTLOCATIONS-Failed: There is no active session. MID = ' . $MID;
                    $isInserted = $apiLogsModel->insertAPIlogs($apiMethod, '', $apiDetails, '', 2);
                    if ($isInserted == 0) {
                        $logMessage = "Failed to insert to APILogs.";
                        $logger->log($logger->logdate, "[LISTLOCATIONS ERROR]: MID " . $MID . " || ", $logMessage);
                ...
                if (count($isExist) > 0) {
                    ...
                    $pegsSiteLocations = $tblPegsSiteInfoModel->getPegsSiteLocations();
                    foreach($pegsSiteLocations as $singlePGSL)
                    {
                        $siteName = $singlePGSL['SiteName'];
                        $siteAddress[] = $singlePGSL['SiteAddress'];
                        foreach($siteAddress as $singleSiteAddress)
                        {
                            $address = $singleSiteAddress.', ';
                        }
                        $barangay = $ref_BarangayModel->getBarangayNameUsingBarangayID($singlePGSL['BarangayID']);
                        foreach($barangay as $singleBarangay)
                        {
                            $address .= $singleBarangay.', ';
                        }
                        $city = $ref_CitiesModel->getCityNameUsingCityID($singlePGSL['CityID']);
                        foreach($city as $singleCity)
                        {
                            $address .= $singleCity.', ';
                        }
                        $province = $ref_ProvincesModel->getProvinceNameUsingProvinceID($singlePGSL['ProvinceID']);
                        foreach($province as $singleProvince)
                        {
                            $address .= $singleProvince.', ';
                        }
                        $region = $ref_RegionsModel->getRegionNameUsingRegionID($singlePGSL['RegionID']);
                        foreach($region as $singleRegion)
                        {
                            $address .= $singleRegion.', ';
                        }
                        $island = $ref_IslandsModel->getIslandNameUsingIslandID($singlePGSL['IslandID']);
                        foreach($island as $singleIsland)
                        {
                            $address .= $singleIsland;
                        }
                        $completeAddress = $address;
                        $contactNumber = $singlePGSL['ContactNumber'];
...
//                        }
                    ...
                    $nonPegsSiteLocations = $tblNonPegsSiteInfoModel->getNonPegsSiteLocations();
                    foreach($nonPegsSiteLocations as $singleNPGSL)
                    {
                        $siteName = $singleNPGSL['fldSiteName'];
                        $siteAddress[] = $singleNPGSL['fldSiteAddress'];
                        foreach($siteAddress as $singleSiteAddress)
                        {
                            $address = $singleSiteAddress.', ';
                        }
                        $barangay = $ref_BarangayModel->getBarangayNameUsingBarangayID($singleNPGSL['fldBarangayID']);
                        foreach($barangay as $singleBarangay)
                        {
                            $address .= $singleBarangay.', ';
                        }
                        $city = $ref_CitiesModel->getCityNameUsingCityID($singleNPGSL['fldCityID']);
                        foreach($city as $singleCity)
                        {
                            $address .= $singleCity.', ';
                        }
                        $province = $ref_ProvincesModel->getProvinceNameUsingProvinceID($singleNPGSL['fldProvinceID']);
                        foreach($province as $singleProvince)
                        {
                            $address .= $singleProvince.', ';
                        }
                        $region = $ref_RegionsModel->getRegionNameUsingRegionID($singleNPGSL['fldRegionID']);
                        foreach($region as $singleRegion)
                        {
                            $address .= $singleRegion.', ';
                        }
                        $island = $ref_IslandsModel->getIslandNameUsingIslandID($singleNPGSL['fldIslandID']);
                        foreach($island as $singleIsland)
                        {
                            $address .= $singleIsland;
                        }
                        $completeAddress = $address;
                        $contactNumber = $singleNPGSL['fldContactNumber'];
...
//                        }
                    ...
                    $locations = array_merge_recursive($pegsSiteLocationsList, $nonPegsSiteLocationsList);
//                    for ($itr = 0; $itr < count($nonPegsSiteLocations); $itr++) {
//                        $siteName[$itr] = $nonPegsSiteLocations[$itr]['fldSiteName'];
//                        $island = $ref_IslandsModel->getIslandNameUsingIslandID($nonPegsSiteLocations[$itr]['fldIslandID']);
//                        $region = $ref_RegionsModel->getRegionNameUsingRegionID($nonPegsSiteLocations[$itr]['fldRegionID']);
//                        $province = $ref_ProvincesModel->getProvinceNameUsingProvinceID($nonPegsSiteLocations[$itr]['fldProvinceID']);
//                        $city = $ref_CitiesModel->getCityNameUsingCityID($nonPegsSiteLocations[$itr]['fldCityID']);
//                        $barangay = $ref_BarangayModel->getBarangayNameUsingBarangayID($nonPegsSiteLocations[$itr]['fldBarangayID']);
//                        $siteAddress[$itr] = $nonPegsSiteLocations[$itr]['fldSiteAddress'];
//                        $contactNumber[$itr] = $nonPegsSiteLocations[$itr]['fldContactNumber'];
//                        $operatingHours[$itr] = $nonPegsSiteLocations[$itr]['fldOperatingHours'];
//                        $latitude = (string)$nonPegsSiteLocations[$itr]['fldLatitude'];
//                        $longitude = (string)$nonPegsSiteLocations[$itr]['fldLongitude'];
//                        //$address[$itr] = $siteAddress[$itr].','.$barangay.','.$city.','.$province.','.$region.','.$island;
//                        //$nonPegsSiteLocationsList[$itr] = array('Address' => $address[$itr], 'TelephoneNumber' => $contactNumber[$itr], 'OperatingHours' => $operatingHours[$itr], 'Latitude' => $latitude[$itr], 'Longitude' => $longitude[$itr], 'SiteName' => $siteName[$itr]);
////                        //$promos[$itr] = array('PromoName' => $promoName, 'PromoDetails' => $promoDetails, 'StartDate' => $startDate, 'EndDate' => $endDate, 'PromoThumbnail' => $promoThumbnail);
////                        //$promosList[$itr] = array_merge($promosList[$itr], $promos[$itr]);
//                        //$nonPegsSiteLocations[$itr] = $nonPegsSiteLocationsList[$itr];
//                    }
//                    exit;
                    $isSuccessful = $auditTrailModel->logEvent(AuditTrailModel::API_LIST_LOCATIONS, 'MID: ' . $MID, array('MID' => $MID, 'SessionID' => $mpSessionID));
                    ...
                    $transMsg = 'No Error, Transaction successful.';
                    $errorCode = 0;
                    $data = CommonController::retMsgListLocations($module, $locations, $errorCode, $transMsg);
                    $message = "[" . $module . "] Output: " . CJSON::encode($data);
                    $appLogger->log($appLogger->logdate, "[response]", $message);
                    $this->_sendResponse(200, CJSON::encode($data));
                    $logMessage = 'List Locations is successful.';
                    $logger->log($logger->logdate, "[LISTLOCATIONS SUCCESSFUL]: MID " . $MID . " || ", $logMessage);
                    $apiDetails = 'LISTLOCATIONS-Successful: MID = ' . $MID;
                    $isInserted = $apiLogsModel->insertAPIlogs($apiMethod, $refID, $apiDetails, '', 1);
                    if ($isInserted == 0) {
                        $logMessage = "Failed to insert to APILogs.";
                        $logger->log($logger->logdate, "[LISTLOCATIONS ERROR]: MID " . $MID . " || ", $logMessage);
                    ...
                    $errorCode = 13;
                    Utilities::log("ReturnMessage: " . $transMsg . " ErrorCode: " . $errorCode);
                    $data = CommonController::retMsgListPromos($module, $locationsList, $errorCode, $transMsg);
                    $message = "[" . $module . "] Output: " . CJSON::encode($data);
                    $appLogger->log($appLogger->logdate, "[response]", $message);
                    $this->_sendResponse(200, CJSON::encode($data));
                    $logMessage = 'MPSessionID does not exist.';
                ...
                if ((Utilities::validateEmail($request['Username']) || ctype_alnum($request['Username'])) && ctype_alnum($request['Password']) && is_numeric($request['AlterStr'])) {
                    ...
                    if ($members) {
                        ...
                        if($result['Status'] != 1 && $result['Status'] != NULL) {
                            ...
                            Utilities::log("ReturnMessage: " . $transMsg . " ErrorCode: " . $errorCode);
                            $data = CommonController::retMsgCreateMobileInfo($module, '', '', '', '', $errorCode, $transMsg, '');
                            $message = "[CREATEMOBILEINFO] Output: " . CJSON::encode($data);
                            $appLogger->log($appLogger->logdate, "[response]", $message);
                            $this->_sendResponse(200, CJSON::encode($data));
                            $logger->log($logger->logdate, "[CREATEMOBILEINFO ERROR]: " . $request['Username'] . " || " . $request['AlterStr'] . " || ", $logMessage);
                            $apiDetails = 'CREATEMOBILEINFO-Failed: Invalid Login parameters.';
                            $isInserted = $apiLogsModel->insertAPIlogs($apiMethod, '', $apiDetails, '', 2);
                            if ($isInserted == 0) {
                                $logMessage = "Failed to insert to APILogs.";
                                $logger->log($logger->logdate, "[CREATEMOBILEINFO ERROR]: " . $request['Username'] . " || " . $request['AlterStr'] . " || ", $logMessage);
                        ...
                        if ($result > 0) {
                            ...
                            if ($isUpdated > 0) {
                                ...
                                $errorCode = 0;
                                $data = CommonController::retMsgCreateMobileInfo($module, $mpSessionID, $cardTypeID, $isVIP, $cardNumber, $errorCode, $transMsg, $remarks);
                                $message = "[" . $module . "] Output: " . CJSON::encode($data);
                                $appLogger->log($appLogger->logdate, "[response]", $message);
                                $this->_sendResponse(200, CJSON::encode($data));
    ...
    public function actionVerifyTracking2() {
        ...
        $encR = $cipher->encryptR($tracking1 . $remarks);
        $transMsg = $encR;
	$data = CommonController::retMsgVerifyTracking2($module,$tracking1,$remarks,$errorCode,$transMsg);
	$this->_sendResponse(200, CJSON::encode($data));
            ...
            if (($mpSessionID == '') || ($cardNumber == '')) {
                ...
                if($resultMID)
                {
                    ...
                    if ($result) {
                        ...
                        $logMessage = 'Cannot access PCWS API.';
                        $logger->log($logger->logdate, "[GETBALANCE ERROR]: " . $cardNumber . " || ", $logMessage);
                        $apiDetails = 'GETBALANCE-Failed: Cannot access PCWS API. Card Number = ' . $cardNumber;
                        $isInserted = $apiLogsModel->insertAPIlogs($apiMethod, '', $apiDetails, '', 2);
                        if ($isInserted == 0) {
                            $logMessage = "Failed to insert to APILogs.";
                            $logger->log($logger->logdate, "[GETBALANCE ERROR]: " . $cardNumber . " || ", $logMessage);
