...
class MPapiInvokerController extends Controller {
    ...
    public function actionListPromos() {
        $this->pageTitle = 'Membership Portal API - List Promos';
        $result = '';

        if (isset($_POST['ViewBy']) || isset($_POST['MPSessionID'])) {
            $viewBy = $_POST['ViewBy'];
            $mpSessionID = $_POST['MPSessionID'];

            $result = $this->_listPromos($viewBy, $mpSessionID);
        }

        $this->render('listpromos', array('result' => $result));
    }
    
    public function actionListLocations() {
        $this->pageTitle = 'Membership Portal API - List Locations';
        $result = '';

        if (isset($_POST['MPSessionID'])) {
            $mpSessionID = $_POST['MPSessionID'];

            $result = $this->_listLocations($mpSessionID);
        }

        $this->render('listlocations', array('result' => $result));
    }
    ...
    private function _listPromos($viewBy, $mpSessionID) {
        $postdata = CJSON::encode(array('ViewBy' => $viewBy, 'MPSessionID' => $mpSessionID));

        $result = $this->SubmitData(Yii::app()->params['urlMPAPI'] . 'listpromos', $postdata);

        return $result[1];
    }
    
    private function _listLocations($mpSessionID) {
        $postdata = CJSON::encode(array('MPSessionID' => $mpSessionID));

        $result = $this->SubmitData(Yii::app()->params['urlMPAPI'] . 'listlocations', $postdata);
