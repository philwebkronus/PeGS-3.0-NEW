...
//
    ...
    public function actionRedeemItems(){
        ...
        if(isset($_POST['TPSessionID']) || isset($_POST['MPSessionID']) || isset($_POST['CardNumber']) || isset($_POST['RewardID']) || isset($_POST['REwardItemID'])|| isset($_POST['Quantity']) || isset($_POST['Source']) || isset($_POST['Tracking1']) || isset($_POST['Tracking2'])){
            ...
            $config = Yii::app()->params['config'];
            $result = $this->_redeemItems($TPSessionID,$MPSessionID,$CardNumber,$RewardID,$RewardItemID,$Quantity,$Source,$Tracking1,$Tracking2,$moduleName,$config);
        }

        $this->render($moduleName, array('result'=>$result));
    }
        private function _redeemItems($TPSessionID,$MPSessionID,$CardNumber,$RewardID,$RewardItemID,$Quantity,$Source,$Tracking1,$Tracking2,$moduleName,$config){
            $url = $this->genURL($moduleName);
            $postData = CJSON::encode(array('TPSessionID'=>$TPSessionID, 'MPSessionID'=>$MPSessionID, 'CardNumber'=>$CardNumber, 'RewardID'=>$RewardID, 'RewardItemID'=>$RewardItemID, 'Quantity'=>$Quantity, 'Source'=>$Source, 'Tracking1' => $Tracking1, 'Tracking2' => $Tracking2, 'Config' => $config));
            $result = $this->SubmitData($url, $postData);
    ...
    public function actionListPromos(){
        $this->pageTitle= $this->genTitlePage('List Promos');

        $result='';
        $moduleName ='listpromos';

        if(isset($_POST['TPSessionID']) && isset($_POST['MPSessionID']) && isset($_POST['ViewBy'])){
            $TPSessionID = $_POST['TPSessionID'];
            $MPSessionID = $_POST['MPSessionID'];
            $viewBy = $_POST['ViewBy'];
            $result = $this->_listPromos($TPSessionID,$MPSessionID,$viewBy,$moduleName);
        }

        $this->render($moduleName, array('result'=>$result));
    }
        
    private function _listPromos($TPSessionID,$MPSessionID,$viewBy,$moduleName){
        $url = $this->genURL($moduleName);
        $postData = CJSON::encode(array('TPSessionID'=>$TPSessionID, 'MPSessionID'=>$MPSessionID, 'ViewBy'=>$viewBy));
        $result = $this->SubmitData($url, $postData);
    ...
    public function actionListLocations(){
        $this->pageTitle= $this->genTitlePage('List Locations');

        $result='';
        $moduleName ='listlocations';

        if(isset($_POST['TPSessionID']) && isset($_POST['MPSessionID'])){
            $TPSessionID = $_POST['TPSessionID'];
            $MPSessionID = $_POST['MPSessionID'];
            $result = $this->_listLocations($TPSessionID,$MPSessionID,$moduleName);
        }

        $this->render($moduleName, array('result'=>$result));
    }
        
    private function _listLocations($TPSessionID,$MPSessionID,$moduleName){
        $url = $this->genURL($moduleName);
        $postData = CJSON::encode(array('TPSessionID'=>$TPSessionID, 'MPSessionID'=>$MPSessionID));
        $result = $this->SubmitData($url, $postData);
