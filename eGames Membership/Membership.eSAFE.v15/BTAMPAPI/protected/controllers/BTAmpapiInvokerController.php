...
class BTAmpapiInvokerController extends Controller {
    ...
    /*
     * Added Date: August 10, 2015
     * @javida
     */
	public function actionRegisterMemberBTNoEmail(){
        $this->pageTitle= $this->genTitlePage('Register Member BT (No Email)');
        $result='';

        $moduleName ='registermemberbtnoemail';

        if(isset($_POST['TPSessionID']) || isset($_POST['FirstName']) || isset($_POST['LastName']) ||isset($_POST['MobileNo']) || isset($_POST['Birthdate'])){
            $TPSessionID = $_POST['TPSessionID'];
            $FirstName = $_POST['FirstName'];
            $LastName = $_POST['LastName'];
            $MobileNo = $_POST['MobileNo'];
            $Birthdate = $_POST['Birthdate'];
            $result = $this->_registerMemberBTNoEmail($TPSessionID, $FirstName, $LastName, $MobileNo, $Birthdate, $moduleName);
        }
     ...
     * @javida
     */
    private function _registerMemberBTNoEmail($TPSessionID, $FirstName, $LastName, $MobileNo, $Birthdate, $moduleName){
        $url = $this->genURL($moduleName);
        $postData = CJSON::encode(array('TPSessionID'=>$TPSessionID,'FirstName'=>$FirstName, 'LastName'=>$LastName, 'MobileNo'=>$MobileNo, 'Birthdate'=>$Birthdate));
        $result = $this->SubmitData($url, $postData);
        return $result[1];
    }
