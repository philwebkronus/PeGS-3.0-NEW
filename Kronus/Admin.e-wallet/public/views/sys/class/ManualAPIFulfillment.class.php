...
class ManualAPIFulfillment extends DBHandler
{
    ...
    */
     ...
     */
      ...
      {
          $stmt = "SELECT TerminalID, TerminalName, TerminalCode FROM terminals WHERE SiteID = '".$zsiteID."' AND TerminalType IN (0,1) ORDER BY TerminalID ASC";
          $this->executeQuery($stmt);
          return $this->fetchAllData();
      }
    ...
    */
    ...
    function getEgmReqID($terminalID, $mid)
     {
           $stmt = "SELECT EGMRequestLogID FROM egmrequestlogs 
                WHERE TerminalID = ? AND MID = ? AND Status  = 0";
           $this->prepare($stmt);
           $this->bindparameter(1, $terminalID);
           $this->bindparameter(2, $mid);
           $this->execute($stmt);
    ...
    public function updateStackerDetailsID($transdetailsid,$stackersummaryid){
        $stackerdetailsid = $this->getStackerDetailsID($stackersummaryid);
         ...
         $this->prepare("UPDATE stackerdetails SET TransactionDetailsID = ?
                                            WHERE StackerDetailID = ?");
    ...
    public function getStackerDetailsID($stackersummaryid) {
        $sql = 'SELECT MAX(StackerDetailID) as StackerDetailID FROM stackermanagement.stackerdetails 
     ...
     */
     ...
     {
         $stmt = "SELECT COUNT(MID) as Count, RequestSource   
                  FROM pendingwallettrans 
                  WHERE MID = ?";
         $this->prepare($stmt);
     ...
     {
         $stmt = "SELECT EwalletTransID, StartDate, MID, Amount, TransType, 
                  ServiceID, Status, UserMode, PaymentType, TerminalID, 
     ...
     public function updateEwalletTransStat($ewallettransid, $status, $tobalance, $converted_res, $transrefid, $transsummaryid, $totalWalletReloads)
     {
         $this->begintrans();
         try
         {
            $stmt = "UPDATE ewallettrans SET Status = ?, 
                                             EndDate = NOW(6), 
                                             ToBalance = ?, 
            ...
            $this->bindparameter(4, $transrefid);
            $this->bindparameter(5, $ewallettransid);
            if ($this->execute() > 0)
            {
                try
                {
