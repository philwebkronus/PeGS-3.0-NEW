...
class CSManagement extends DBHandler{
     ...
     //check card number
      function chkcardnumber($zCardNumber){
           $this->begintrans();
           $this->prepare("SELECT COUNT(CardNumber) count, Status, MID from membercards WHERE CardNumber = ?");
           $this->bindparameter(1, $zCardNumber);
           $this->execute();
           return $this->fetchData();
      }
      
      //reset player pin
      function resetplayerpin($zMID){
          $this->begintrans();
          $this->prepare("UPDATE members SET PIN = ?, DatePINLastChange=NULL WHERE MID = ?");
          $this->bindparameter(1, sha1('1234'));
          $this->bindparameter(2, $zMID);
          if($this->execute()){   
            return 1;
          }
          else{
