...
class ManualAPIFulfillment extends DBHandler
{
     ...
     */
      function getTransactionReferenceIDterminal($terminal){
            $stmt = "SELECT TransactionReferenceID, LoyaltyCardNumber, RequestSource FROM pendingterminaltransactions WHERE TerminalID = ?";
            $this->prepare($stmt);
            $this->bindparameter(1,$terminal);
            $this->execute();
     ...
     */
      function getTransactionReferenceIDuser($terminal){
            $stmt = "SELECT TransactionReferenceID, LoyaltyCardNumber, RequestSource FROM pendingusertransactions WHERE TerminalID = ?";
            $this->prepare($stmt);
            $this->bindparameter(1,$terminal);
            $this->execute();
            return $this->fetchAllData();      
    ...
    */
      function gettransactionRequestlogs($trefid){
            $stmt = "SELECT Amount, TransactionReferenceID, TransactionType, ServiceID, UserMode FROM transactionrequestlogs WHERE TransactionReferenceID = ?";
            $this->prepare($stmt);
     ...
     */
      function gettransactionRequestlogslp($trefid){
            $stmt = "SELECT Amount, TransactionReferenceID, TransactionType, ServiceID, UserMode, TransactionSummaryID FROM transactionrequestlogslp WHERE TransactionReferenceID = ?";
            $this->prepare($stmt);
            $this->bindparameter(1,$trefid);
            $this->execute();
            return $this->fetchAllData();      
     ...
     */
      ...
      /**
     * @author Gerardo V. Jagolino Jr.
     * @param $status, $transrefid, $servicetransid, $sevicestatus
     * @return array
