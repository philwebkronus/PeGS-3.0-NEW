<?php 
$pagetitle = "Confirmation History";
include "header.php";
...
<script type="text/javascript">
    jQuery(document).ready(function(){
        jQuery('#btnpdf').click(function(){
            jQuery('#frmexport').attr('action','process/ProcessTopUpGenerateReports.php?action=confirmationpdf');
            jQuery('#frmexport').submit();            
        });
        
        jQuery('#btnexcel').click(function(){
            jQuery('#frmexport').attr('action','process/ProcessTopUpGenerateReports.php?action=confirmationexcel');
            jQuery('#frmexport').submit();  
        });        
        
        jQuery('#tblreplenish').jqGrid({
            url : 'process/ProcessTopUpPaginate.php?action=confirmation',
            datatype: "json",
            colNames:['Account Name', 'Site / PEGS Code', 'POS Account','Date Credited','Date Created', 'Who', 'Amount', 'Created By'],
            rowNum:10,
            height: 280,
            width: 1200,
            rowList:[10,20,30],
            ...
            viewrecords: true,
            sortorder: "asc",
            caption:"Confirmation History",            
            colModel:[
                {name:'AccountName',index:'UserName',align:'left'},
                {name:'SiteCode',index:'SiteCode',align:'left'},
                {name:'POSAccountNo',index:'POSAccountNo',align:'center'},
                {name:'DateCredited',index:'DateCredited',align:'left'},
                {name:'DateCreated',index:'DateCreated',align:'left'},
                {name:'Who',index:'SiteRepresentative',align:'left'},
                {name:'Amount',index:'AmountConfirmed',align:'right'},
                {name:'CreatedBy',index:'UserName',align:'left'},
            ]
        });
        ...
        jQuery('#btnsearch').click(function(){
            ...
            jQuery("#tblreplenish").jqGrid('setGridParam',{url:"process/ProcessTopUpPaginate.php?action=confirmation&startdate="+startdate+
