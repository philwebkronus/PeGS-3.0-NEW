...
class TerminalSessionsModel extends MI_Model {
    ...
    public function getEwalletTerminal($siteID){
        $sql = "SELECT t.TerminalCode, ts.LoyaltyCardNumber FROM npos.terminalsessions ts INNER JOIN membership.members m ON ts.MID=m.MID INNER JOIN npos.terminals t ON t.TerminalID=ts.TerminalID WHERE ts.ServiceID=19 AND m.IsEwallet=1 AND t.SiteID=:siteID ORDER BY t.TerminalCode";
