...
class RptPegsOps extends DBHandler{
    ...
    /********** FOR TERMINAL LISTING *********/ 
          ...
          if($zstart == null && $zlimit == null)
          {
              ...
              if($zsiteID > 0)
              {
                 $stmt = "SELECT DISTINCT s.SiteName as 'Site Name',s.SiteCode as 'Site Code',
                          s.POSAccountNo as 'POS Account No.',a.UserName as 'User Name',at.Name as 'User Group',
                          ...
                          WHERE sa.Status = 1 AND s.SiteID = ? AND s.SiteID <>1 "; 
                 $this->prepare($stmt);
                 $this->bindparameter(1, $zsiteID);
              }
              else
              {
                 $stmt = "SELECT DISTINCT s.SiteName as 'Site Name',s.SiteCode as 'Site Code',
                          ...
                          INNER JOIN sites s ON s.SiteID = sa.SiteID
                          INNER JOIN ref_accounttypes at ON at.AccountTypeID = a.AccountTypeID   
                          WHERE sa.Status = 1 AND s.SiteID <>1"; 
                 $this->prepare($stmt); 
              }
          }
          else
      ...
      function countuserlist($zsiteID)
      {
          if($zsiteID > 0)
          {
              $stmt = "SELECT COUNT(DISTINCT AID, SiteID) as ctruser FROM siteaccounts 
                       WHERE Status = 1 AND SiteID <> 1 AND SiteID = ?";
              $this->prepare($stmt);
              $this->bindparameter(1, $zsiteID);
          }
          else
