			...
			if (!$this->empty_string($fontfile)) {
				...
				if ((!$compressed) AND (isset($info['length2']))) {
					...
					//$font = $this->_getTrueTypeFontSubset($font, $subsetchars);
                                        ...
                                        // alcal: $font2cache modification (tcpdf.php 5.5.014)
                                        ...
                                        // see http://en.wikipedia.org/wiki/Unicode_typefaces

                                        // subset of Latin, Latin-1 Supplement, Latin Extended-A, Latin Extended-B
                                        $subsetchars = array_fill(0, 591, true); 

                                        // add Greek and Coptic 0370–03FF, 143 chars
                                        for ($i = 880; $i <= 1023; $i++) {
                                        $subsetchars[$i] = true;
                                        }

                                        // add Cyrillic: U+0400..U+04FF, 256 chars
                                        for ($i = 1024; $i <= 1279; $i++) {
                                        $subsetchars[$i] = true;
                                        }
