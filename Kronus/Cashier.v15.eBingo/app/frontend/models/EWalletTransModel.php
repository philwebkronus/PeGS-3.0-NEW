...
class EWalletTransModel extends MI_Model {
    public function getEWalletTransactionPerSite($startDate, $endDate, $siteID, $limit=null){
        ...
        $sql = "SELECT ew.StartDate, ew.LoyaltyCardNumber, ew.Amount, ew.TransType, SUBSTR(t.TerminalCode,$len) as TerminalCode, 
                    ...
                    AND ew.Status IN (1,3)  AND ew.SiteID=:siteID ORDER BY ew.StartDate DESC".(!empty($limit)?" LIMIT $limit":"");
        $param1 = array(
    ...
    public function getEWalletTransactionPerSiteTotal($startDate, $endDate, $siteID, $aid, $limit=null){
        ...
        $sql = "SELECT SUM(Amount) as Amount,  TransType FROM ewallettrans WHERE StartDate >= :startDate AND StartDate < :endDate 
                    AND Status IN (1,3)  AND SiteID=:siteID GROUP BY TransType".(!empty($limit)?" LIMIT $limit":"");
    ...
    public function getEWalletTransactionPerCashier($startDate, $endDate, $siteID, $aid, $limit=null){
        ...
        $sql = "SELECT ew.StartDate, ew.LoyaltyCardNumber, ew.Amount,  ew.TransType, SUBSTR(t.TerminalCode,$len) as TerminalCode, 
                    ...
                    LEFT JOIN terminals t ON ew.TerminalID = t.TerminalID 
                    WHERE ew.StartDate >= :startDate AND ew.StartDate < :endDate 
                    AND ew.Status IN (1,3) AND ew.SiteID=:siteID AND ew.CreatedByAID=:aid ORDER BY ew.StartDate DESC".(!empty($limit)?" LIMIT $limit":"");
    ...
    public function getEWalletTransactionPerCashierTotal($startDate, $endDate, $siteID, $aid, $limit=null){
        ...
        $sql = "SELECT SUM(Amount) as Amount,  TransType FROM ewallettrans WHERE StartDate >= :startDate AND StartDate < :endDate 
                    AND Status IN (1,3) AND SiteID=:siteID AND CreatedByAID=:aid GROUP BY TransType".(!empty($limit)?" LIMIT $limit":"");
    ...
    public function geteSAFELoadsAndWithdrawals($startdate,$enddate,$siteid){
        $cutoff_time = Mirage::app()->param['cut_off'];
        $result = array();
        $eSAFELoadCashsql="SELECT IFNULL(SUM(Amount),0) as eSAFELoadCash FROM ewallettrans WHERE StartDate >= :startdate AND StartDate < :enddate
        ...
        $eSAFELoadBancnetsql="SELECT IFNULL(SUM(Amount),0) as eSAFELoadBancnet FROM ewallettrans WHERE StartDate >= :startdate AND StartDate < :enddate
                                                AND SiteID = :siteid AND PaymentType = 1 AND Status IN (1,3) AND TransType='D' 
                                                AND TRIM(IFNULL(TraceNumber, '')) > '' AND TRIM(IFNULL(ReferenceNumber, '')) > ''";
        $eSAFELoadCouponsql="SELECT IFNULL(SUM(Amount),0) as eSAFELoadCoupon FROM ewallettrans WHERE StartDate >= :startdate AND StartDate < :enddate
                                                AND SiteID = :siteid AND PaymentType = 2 AND Status IN (1,3) AND TransType='D' 
                                                AND (TraceNumber IS NULL OR TraceNumber = '') AND (ReferenceNumber IS NULL OR ReferenceNumber = '')";
        $eSAFELoadTicketsql="SELECT IFNULL(SUM(Amount),0) as eSAFELoadTicket FROM ewallettrans WHERE StartDate >= :startdate AND StartDate < :enddate
                                                AND SiteID = :siteid AND PaymentType = 3 AND Status IN (1,3) AND TransType='D' 
                                                AND (TraceNumber IS NULL OR TraceNumber = '') AND (ReferenceNumber IS NULL OR ReferenceNumber = '')";
        $eSAFECashierRedemptionsql="SELECT IFNULL(SUM(Amount),0) as eSAFECashierRedemption FROM ewallettrans WHERE StartDate >= :startdate AND StartDate < :enddate
                                                AND SiteID = :siteid AND PaymentType = 1 AND Status IN (1,3) AND TransType='W' 
                                                AND (TraceNumber IS NULL OR TraceNumber = '') AND (ReferenceNumber IS NULL OR ReferenceNumber = '')
                                                AND Source = 0";
        $eSAFEGenesisRedemptionsql="SELECT IFNULL(SUM(Amount),0) as eSAFEGenesisRedemption FROM ewallettrans WHERE StartDate >= :startdate AND StartDate < :enddate
    ...
    public function CheckIfeSAFETrans($transsumid){
        $checkifesafetrans = "SELECT COUNT(EwalletTransID) as esafecounter FROM ewallettrans WHERE TransactionSummaryID = :transsumid";
        $param = array(
