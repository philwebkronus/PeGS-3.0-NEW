...
class LoyaltyAPIWrapper {
    ...
    public function transferPoints($oldnumber,$newnumber,$aid,$return_transfer=false) {
        ...
        $ch = curl_init();
        curl_setopt($ch,CURLOPT_URL, Mirage::app()->param['transfer_points'] . '?oldnumber=' . $oldnumber.'&newnumber='.$newnumber.'&aid='.$aid);
        curl_setopt( $ch, CURLOPT_FRESH_CONNECT, FALSE );
        curl_setopt( $ch, CURLOPT_CONNECTTIMEOUT, 10 );
        curl_setopt( $ch, CURLOPT_TIMEOUT, 500 );
        curl_setopt( $ch, CURLOPT_USERAGENT, 'PEGS Station Manager' );
                                                                ...
                                                                $service_id = 7,$return_transfer=false) 
    {
        ...
        $ch = curl_init();
        curl_setopt($ch,CURLOPT_URL, Mirage::app()->param['process_points'] . '?cardnumber=' . $card_number.'&transactionid='.$transid.'&transdate='.$transdate.
