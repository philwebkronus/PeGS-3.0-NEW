...
$oas= new ApplicationSupport($_DBConnectionString[0]); //npos
$oas2= new ApplicationSupport($_DBConnectionString[2]); //stacker
$oas3= new ApplicationSupport($_DBConnectionString[4]); //loyalty instance
                 ...
                 && isset($_POST['cmbstatus']))
                 {
                        ...
                        if(count($result) > 0)
                        {
                             ...
                             {                     
                                ...
                                $responce->rows[$i]['cell']=array($vview['EwalletTransID'], 
                                                                  $results[1], 
                                                                  $vtranstype, 
                ...
                if((isset ($_POST['txtcardnumber']) || isset($_POST['cmbsite'])) 
                 ...
                 && isset($_POST['cmbstatus']))
                 {
                        ...
                        if ($cardnumber != "" || $site != -1)
                        {
                            ...
                            $rcount = $oas->countselectewallettrans($vtransstatus, $vFrom,$vTo, $cardnumber, $site); 
                            ...
                            $start = $limit * $page - $limit;
                            $limit = (int)$limit;   
                            $result = $oas->selectewallettransdtls($vtransstatus, $vFrom,$vTo, $start, $limit, $cardnumber, $site);  
                            if(count($result) > 0)
                            {
                                 $i = 0;
                                 $responce->page = $page;
                                 ...
                                 {    
                                    ...
                                    switch ($vview['UserMode']) {
                                        ...
                                        // CCT ADDED 01/25/2018 BEGIN
                                        case 4: $usermode = 'Terminal Based'; break;
                                        // CCT ADDED 01/25/2018 END
                                        default:
                                            break;
                                    }

                                    $name = $oas->getNamebyAid($vview['UpdatedByAID']);

                                    $results2 = $oas->getsitecode($vview['SiteID']);
                                    $results2 = $results2['SiteCode'];
                                    //get terminal code
                                    if (!is_null($vview['TerminalID']))
                                    {
                                        $getTerminalCode = $oas->getTerminalCode($vview['TerminalID']);
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                $responce->total = $total_pages;
                                $responce->records = $count;
                                $msg = "Application Support: Please enter card number or enter site.";
                                $responce->msg = $msg;
                            }
                        }
                        echo json_encode($responce);
        ...
        switch ($vpage2)
        {
            ...
            case 'UnlockMGTerminal':
               ...
               unset($vapiResult,$vsiteID, $vterminal, $vserviceID, $vsitecode, 
                     ...
                     $rservicegrpID, $url, $login, $roldsite, 
                     // EDITED CCT 02/06/2018 END
            ...
            case 'UnlockMGTerminalUB':
                ...
                else
                {
                    if(isset($_POST['txtcardnumber']) && isset($_POST['cmbnewservice']))
                    {
                        ...
                        switch (true){
                                // Comment Out CCT 02/06/2018 BEGIN
                                //ase strstr($servername, "MG"): //if provider is MG, then
                                //    $_MGCredentials = $_PlayerAPI[$vserviceID -1]; 
                                //    list($mgurl, $mgserverID) = $_MGCredentials;
                                //    $url = $mgurl;
                                //    $hashedPassword = '';
