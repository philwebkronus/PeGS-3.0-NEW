...
if($connected)
{
    ...
    {
        $usermode = $ortg->checkUserMode($_SESSION['serverid']);
        if($usermode == 2) 
        {
            $configuration = array( 'URI' => $_ServiceAPI[$serverId-1],
                                   'isCaching' => FALSE,
    ...
    if ( (bool)$_CasinoAPIHandler->IsAPIServerOK() )
    {
       ...
       if(isset ($_SESSION['Withdraw']) == 'Withdraw')
       {
           ...
           $repamount = ereg_replace(",", "", $_SESSION['txtamount']);;
           ...
           {  
               ...
               $redeemedAmount = number_format($_maxRedeem,2, '.', ','); 
               // CCT ADDED 12/20/2017 END
           }
           else
           {
           ...
           if($rwithdraw['IsSucceed'] == true )
           {
                ...
                // EDITED CCT 12/20/2017 BEGIN
                //$vreportedAmt = $ramount;
                $vreportedAmt = $repamount;
                // EDITED CCT 12/20/2017 END
                $vactualAmt = $rwamount;
                ...
                if($riserror == "OK")
                {
                    ...
                    if($issucess > 0)
                    {
                       $ortg->updateTerminalSessions($balance, $vterminalID);
                       //insert into audit trail
                       // EDITED CCT 12/20/2017 BEGIN
                       //$vtransdetails = "transaction id ".$vtransactionID.",amount ".$vreportedAmt;
                       $vtransdetails = "transaction id ".$vtransactionID.",amount ".$redeemedAmount;
                       // EDITED CCT 12/20/2017 END
                       $vauditfuncID = 7;
