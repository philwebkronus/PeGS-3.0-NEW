...
class ManualAPIFulfillment extends DBHandler
{
    ...
    */
    ...
    {
        $stmt = "SELECT COUNT(TerminalID) chckpndtertrans from pendingterminaltransactions WHERE TerminalID = ?";
        $this->prepare($stmt);
        $this->bindparameter(1,$terminal);
        $this->execute();
        return $this->fetchData();      
    ...
    */
    ...
    {
        $stmt = "SELECT TransactionReferenceID, TransactionRequestLogID, LoyaltyCardNumber, RequestSource, ServiceID 
    ...
    */
    function insertTransactionreqlogslp($transrefid, $amount, $transtype, $terminalid, $status, $site, $servicetransid, $servicestatus, 
            $serivceid, $servicetransferhisid, $loyaltycard, $mid, $usermode, $transsummaryid, $paymenttype)
    {
        $this->begintrans();
        try
        {
            $stmt = "INSERT INTO transactionrequestlogslp(TransactionReferenceID,Amount,StartDate,EndDate,TransactionType,TerminalID,Status,SiteID, 
                        ServiceTransactionID,ServiceStatus,ServiceID, ServiceTransferHistoryID, LoyaltyCardNumber, MID,UserMode,TransactionSummaryID, 
                        PaymentType) 
    ...
    */
    function gettranslogslpdetails($transtype, $terminal, $service, $transloglpid)
    {
        $stmt = "SELECT ServiceTransferHistoryID, TransactionSummaryID 
                FROM transactionrequestlogslp 
                WHERE TransactionType = ? AND TerminalID = ? AND ServiceID = ? AND TransactionRequestLogLPID = ?";
        $this->prepare($stmt);
        ...
        $trans_type,$service_id,$trans_status, $loyalty_card, $mid, $tdtls_trans_ref_id) // CCT - Added $tdtls_trans_ref_id
    {
        ...
        try
        {
            ...
            if($this->execute()) 
            {
                ...
                $stmt2 = "INSERT INTO transactiondetails (TransactionReferenceID, TransactionSummaryID, SiteID, TerminalID, 
                            TransactionType, Amount, DateCreated, ServiceID, CreatedByAID, Status, LoyaltyCardNumber, MID, PaymentType) 
                        VALUES (?, ?, ?, ?, ?, ?, now_usec(), ?, ?, ?, ?, ?, 1)";
                $this->prepare($stmt2);
                // CCT - BEGIN
    ...
    * @return boolean 
    */
    public function reloadTransaction($amount, $trans_summary_id, $trans_ref_id, $site_id, $terminal_id, $trans_type, $service_id, $acctid, 
            $trans_status, $terminal_balance, $total_terminal_balance, $loyalty_card, $mid, $tdtls_trans_ref_id)     // CCT - Added $tdtls_trans_ref_id
    {
        $this->begintrans();
        try
        {
            ...
            if($this->execute()) 
            {
                $stmt2 = "INSERT INTO transactiondetails (TransactionReferenceID, TransactionSummaryID, SiteID, TerminalID, TransactionType, 
    ...
    * @return boolean 
    */
    public function redeemTransaction($amount, $trans_summary_id, $trans_ref_id, $site_id, $terminal_id, $trans_type, $service_id, $acctid, 
        $trans_status, $loyalty_card, $mid, $tdtls_trans_ref_id) // CCT - Added $tdtls_trans_ref_id
    {
        $this->begintrans();
        try 
        {
            $stmt1 = "UPDATE transactionsummary SET Withdrawal = ?, DateEnded = now_usec() WHERE TransactionsSummaryID = ?";
            $this->prepare($stmt1);
    ...
    */
    ...
    {
        $sql = 'SELECT TransactionsSummaryID, Reload, Withdrawal FROM transactionsummary '
                . ' WHERE SiteID = ? AND TerminalID = ? AND DateEnded = 0 ORDER BY TransactionsSummaryID DESC LIMIT 1';
        $this->prepare($sql);
        $this->bindparameter(1, $site_id);
        $this->bindparameter(2, $terminal_id);
        $this->execute();
    ...
    */
    public function updateBcf($newbal, $site_id, $transdtl) 
    {
        ...
        $sql = 'UPDATE sitebalance SET Balance = Balance - ?, LastTransactionDate = now_usec(), '
                . ' LastTransactionDescription = ? WHERE SiteID = ?';
        //CCT 09/13/2018 END
        $this->prepare($sql);
    ...
    */
    public function insertServiceTransRef($serviceid, $originid) 
    {
        ...
        try 
        {
            $stmt = "INSERT INTO servicetransactionref (ServiceID, TransactionOrigin, DateCreated) VALUES (?, ?, now_usec())";
            $this->prepare($stmt);
            $this->bindparameter(1, $serviceid);
    ...
    */
    public function insert($terminal_id,$service_id,$amount,$trans_summary_id, $loyalty_card, $mid, $user_mode, 
            $casino_login, $casino_pwd, $hashedcasino_pwd) 
    {
        $this->begintrans();
        try 
        {
            $stmt = "INSERT INTO terminalsessions (TerminalID, ServiceID, DateStarted, LastBalance, LastTransactionDate, 
                        TransactionSummaryID, LoyaltyCardNumber, MID, UserMode, UBServiceLogin, UBServicePassword, UBHashedServicePassword) 
                    VALUES (?, ?, now_usec(), ?, now_usec(), ?, ?, ?, ?, ?, ?, ?)";
            $this->prepare($stmt);
            $this->bindparameter(1, $terminal_id);
            $this->bindparameter(2, $service_id);
    ...
    {
        $sql = 'SELECT rsg.ServiceGroupName FROM ref_services rs INNER JOIN ref_servicegroups rsg ON rs.ServiceGroupID = rsg.ServiceGroupID 
    ...
    * @param $terminal_id
    * @return array
    * check check terminal tyee of a specific terminal
    */
    public function checkTerminaType($terminal_id)
    {
        $sql = "SELECT TerminalType FROM terminals t WHERE TerminalID = ?";
        $this->prepare($sql);
        $this->bindparameter(1, $terminal_id);
        $this->execute();
        return $this->fetchData();
    ...
    /**
    * @author Gerardo V. Jagolino Jr.
    * @param $terminalid, $mid, $serviceid, $aid
    * @return last inserted id 
    * Description: insert EGM Sessions
    */
    public function insertEgmSession($terminalid, $mid, $serviceid, $aid) 
    {
        $this->begintrans();
        try 
        {
            $stmt = "INSERT INTO egmsessions (TerminalID, MID, ServiceID, DateCreated, CraetedByAID) 
                    VALUES (?, ?, ?, now(6), ?)";
            $this->prepare($stmt);
            $this->bindparameter(1, $terminalid);
            $this->bindparameter(2, $mid);
            $this->bindparameter(3, $serviceid);
            $this->bindparameter(4, $aid);
    ...
    public function updateEgmRequestLogs($status, $trans_ref_id, $egmreqid) 
    {
        $this->prepare("UPDATE egmrequestlogs SET Status = ?, TransactionReferenceID = ?, DateLastUpdated = now(6) WHERE EGMRequestLogID = ?");
        $this->bindparameter(1, $status);
        $this->bindparameter(2, $trans_ref_id);
        $this->bindparameter(3, $egmreqid);
        $this->execute();
        return $this->rowCount();       
    }

    function getEgmReqID($terminalID, $mid)
    {
        $stmt = "SELECT EGMRequestLogID FROM egmrequestlogs WHERE TerminalID = ? AND MID = ? AND Status  = 0";
        $this->prepare($stmt);
        $this->bindparameter(1, $terminalID);
        $this->bindparameter(2, $mid);
        $this->execute($stmt);
        $count =  $this->fetchData();
        return $count['EGMRequestLogID'];
    }
    ...
    * @param $terminalID
    * @return boolean
    * Description: Deltete egmsession
    */
    public function deleteEGMSession($terminalID) 
    {
        $sql = 'DELETE FROM egmsessions WHERE TerminalID = ?';
        $this->prepare($sql);
        $this->bindparameter(1, $terminalID);
        return $this->execute();
    }

    public function updateSSStatus($aid,$stackerbatchid)
    {
        $this->prepare("UPDATE stackersummary SET Status = 5, UpdatedByAID = ?, DateUpdated = now(6) WHERE Status = 0 AND StackerSummaryID = ?");
        $this->bindparameter(1, $aid);
        $this->bindparameter(2, $stackerbatchid);
        $this->execute();
        return $this->rowCount();
    }

    public function updateStackerDetailsID($transdetailsid,$stackersummaryid)
    {
        $stackerdetailsid = $this->getStackerDetailsID($stackersummaryid);
        $this->prepare("UPDATE stackerdetails SET TransactionDetailsID = ? WHERE StackerDetailID = ?");
        $this->bindparameter(1, $transdetailsid);
        $this->bindparameter(2, $stackerdetailsid);
        $this->execute();
        return $this->rowCount();
    }

    public function getStackerDetailsID($stackersummaryid) 
    {
        $sql = 'SELECT MAX(StackerDetailID) as StackerDetailID FROM stackermanagement.stackerdetails WHERE StackerSummaryID = ?';
        $this->prepare($sql);
        $this->bindparameter(1, $stackersummaryid);
        $this->execute($sql);
        $result =  $this->fetchData();
    ...
    */
    function getStackerBatchID($terminalID)
    {
        $stmt = "SELECT StackerBatchID FROM egmsessions WHERE TerminalID IN (?) ";
        $this->prepare($stmt);
        $this->bindparameter(1, $terminalID);
        $this->execute($stmt);
        $count =  $this->fetchData();
    ...
    * @return type
    */
    function checkPendingEwalletTrans($MID) 
    {
        $stmt = "SELECT COUNT(MID) as Count, RequestSource FROM pendingwallettrans WHERE MID = ?";
        $this->prepare($stmt);
    ...
    * @return type $MID MID of player
    */
    function checkMemberCardExist($cardnumber)
    {
        $stmt = "SELECT MID FROM loyaltydb.membercards WHERE CardNumber = ?";
        $this->prepare($stmt);
    ...
    /**
    * Get Ewallet Transaction Info
    * @param type $MID
    * @return type
    */
    public function getEwalletTransInfo ($MID) 
    {
        $stmt = "SELECT EwalletTransID, StartDate, MID, Amount, TransType, ServiceID, Status, UserMode, PaymentType, TerminalID, 
                    Amount, SiteID, TerminalID  
                FROM ewallettrans 
                WHERE MID = ? AND Status = 0 LIMIT 0, 1";
        $this->prepare($stmt);
        $this->bindparameter(1, $MID);
        $this->execute();
    ...
    public static function stringTransType($transType)
    {
        switch ($transType)
        {
            case 'D': 
    ...
    public function updateEwalletTransStat($ewallettransid, $status, $tobalance, $converted_res, $transrefid, $transsummaryid, $totalWalletReloads, $AID)
    {
        $this->begintrans();
        try
        {
            $stmt = "UPDATE ewallettrans SET Status = ?, EndDate = NOW(6), ToBalance = ?, ServiceTransactionStatus = ?, 
                        ServiceTransactionID = ?, UpdatedByAID = ? 
                    WHERE EwalletTransID = ?";
            $this->prepare($stmt);
            $this->bindparameter(1, $status);
            $this->bindparameter(2, $tobalance);
            $this->bindparameter(3, $converted_res);
            $this->bindparameter(4, $transrefid);
            $this->bindparameter(5, $AID);
    ...
    /**
    * Update BCF for Ewallet Instance
    * @param type $newbal
    * @param type $site_id
    * @param type $transdtl
    * @return type
    * @author MGE
    */
    public function updateBcfEwallet($newbal, $site_id, $transdtl) 
    {
        $this->begintrans();
        try
        {
            $sql = 'UPDATE sitebalance SET Balance = ?, LastTransactionDate = now_usec(), LastTransactionDescription = ? WHERE SiteID = ?';
            $this->prepare($sql);
            $this->bindparameter(1, $newbal);
            $this->bindparameter(2, $transdtl);
            $this->bindparameter(3, $site_id);
    ...
    /**
    * Get FromBalance by EwalletTransID
    * @param type $ewallettransid
    * @return type
    */
    public function getFromBalance($ewallettransid)
    {
        $stmt = "SELECT FromBalance FROM ewallettrans WHERE EwalletTransID = ?";
        $this->prepare($stmt);
        $this->bindparameter(1, $ewallettransid);
        $this->execute();
        $result = $this->fetchData();
        return $result;
    }
    ...
    /**
    * Get TransactionSummaryID from ewallettrans table
    * @param type $ewallettransid
    * @return type array TransactionSummaryID
    */
    public function getTransSummaryID ($ewallettransid)
    {
        $stmt = "SELECT TransactionSummaryID FROM ewallettrans WHERE EwalletTransID = ?";
        $this->prepare($stmt);
        $this->bindparameter(1, $ewallettransid);
        $this->execute();
        $result = $this->fetchData();
    ...
    /**
    * Get Wallet Reloads From transactionsummary
    * @param type $transsummaryid
    * @return type
    */
    public function getWalletReloads ($transsummaryid)
    {
        $stmt = "SELECT WalletReloads FROM transactionsummary WHERE TransactionsSummaryID = ?";
        $this->prepare($stmt);
