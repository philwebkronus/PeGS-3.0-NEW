...
//      }
      ...
      {
          $stmt = "SELECT TerminalID, TerminalName, TerminalCode FROM terminals WHERE SiteID = '".$zsiteID."' ORDER BY TerminalCode ASC";
          $this->executeQuery($stmt);
          return $this->fetchAllData();
      }
     ...
     function viewterminals($zterminalid)
      {
          if($zterminalid > 0)
          {
              $stmt = "SELECT TerminalID, TerminalName, TerminalCode, SiteID, Status, isVIP FROM terminals WHERE TerminalID = '".$zterminalid."' ORDER BY TerminalCode ASC";
              $this->executeQuery($stmt);
              $this->_row = $this->fetchAllData();
          }
          else
          {
              $stmt = "SELECT TerminalID, TerminalName, TerminalCode, SiteID,Status FROM terminals ORDER BY TerminalCode ASC";
              $this->executeQuery($stmt);
              $this->_row = $this->fetchAllData();
          }
          return $this->_row;
      ...
      */
      function viewterminalsbyTerminalCode($zterminalname)
      {
              ...
              $stmt = "SELECT TerminalID FROM terminals WHERE TerminalCode = '".$zterminalname."' ORDER BY TerminalCode ASC";
              $this->executeQuery($stmt);
              $this->_row = $this->fetchAllData();
     ...
     function viewterminaltype($zterminalid)
      {
              $stmt = "SELECT TerminalType FROM terminals WHERE TerminalID = '".$zterminalid."' ORDER BY TerminalCode ASC";
              $this->executeQuery($stmt);
              $this->_row = $this->fetchAllData();
...
//           $stmt = "SELECT b.UserMode 
//                    FROM terminalservices a
//                    INNER JOIN ref_services b
//                    ON a.ServiceID = b.ServiceID
//                    WHERE a.TerminalID = ? AND a.Status = 1
//                    ORDER BY a.TerminalID ASC";
//           $this->prepare($stmt);
//           $this->bindparameter(1, $terminalid);
//           $this->execute($stmt);
