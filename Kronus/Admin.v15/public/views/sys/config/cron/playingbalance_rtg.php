...
include_once ROOT_DIR . 'sys/class/CasinoAPIHandler.class.php';
require_once "../../class/class.export_excel.php";
...
if($connected)
{
    $vtermsession = $oplayingbal->getTerminalSessionsRTG();
    $vctr = 0;
    $varrtermbal = array();
    while($vctr < count($vtermsession))
    {
        ...
        if($connected)
        {
            ...
            if($vservicename['Code']=='MM')
            {
                //connect to RTG
                if(strpos($_ServiceAPI[$vserviceid-1], 'ECFDEMO') === false)
                {
                    $gdeposit = 502;
                    $gwithdraw = 503; 
                }
                ...
                {
                    $vpbalance = $_CasinoAPIHandler->GetBalance( $vterminalcode['TerminalCode'] );
    ...
    $my_file = "playingbalance_rtg.xls";
    $my_path = ROOT_DIR. 'sys/config/cron/'; 
