...
class LoginFormModel extends MI_Model{
    ...
    public function authenticate() {
        ...
        $isterminated = $accountsModel->checktermstatus($this->username);
        if($isterminated > 0){
            session_destroy();
            $this->close();
            $this->setAttributeErrorMessage('message', "Access Denied.You do not allow to access the system.");
            return false;
        }
        ...
        $attempt_count++;
        $accountsModel->updateOnLogin($attempt_count, $date, $this->username);            
        ...
        if(!isset($_SESSION['smacid']) && $_SESSION['smacid'] == '') {
...
//        $_SESSION['chrome'] = $chrome; 
    ...
    public function authenticatePasskey() {
        ...
        if($isadded > 0 || $iscomputerexist['CashierMachineInfoId_PK'] > 0) {
                  ...
                  $auditTrailModel->logToAudit($new_sessionid, $aid, $transdetails, $date, gethostbyaddr($_SERVER['REMOTE_ADDR']), '1');
                  $this->close();
                  $_SESSION['success_login'] = true;
                  return true;
        }
    ...
    public function logout() {
        ...
        $auditTrailModel->logToAudit($sessionid, $aid, $transdetails, $date, $ipaddress, 2);
        $accountSessionsModel->deleteSession($aid);
        session_destroy();
        $this->close();
    }
