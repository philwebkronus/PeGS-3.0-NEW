 ...
 */
    ...
    $stmt = "SELECT p.AID,a.UserName, MAX(p.DateChanged) as DateChanged from passwordcheck p
            ...
            WHERE a.Status = 1 AND a.AccountTypeID NOT IN (15,17)
    ...
    if($vrescount > 0)
    {
        ...
        while($vcounter < $vrescount)
        {
            ...
            //send a notification email to the user 5 days before expiration
            if($noofdays >= 25 && $noofdays < 30)
            {
                $vmessage = "
                           ...
                           <body>
                                <i>Hi $vusername</i>,
                                <br/><br/>
                                    Your password will be expired soon.
                                <br/><br/>
    ...
    unset($stmt,$result,$vrescount,$vcounter,$vaid,$vdatechange,$datenow,$vdiffdate,
          $vctrdate,$noofdays,$resemail,$acctypeid,$updateurl,$vemail,$vto,$vusername,
          $vsubject,$vtime,$vmessage,$vnewhashedpass,$vheaders,$vsentEmail);
