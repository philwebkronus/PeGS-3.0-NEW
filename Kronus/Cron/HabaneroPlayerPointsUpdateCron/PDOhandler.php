...
class PDOhandler {
    ...
    public function getPlayerVvPoints() {
        $query = $this->_connection->prepare('SELECT MID, CardNumber, VvRaffleEntry FROM loyaltydb.membercards WHERE Status = 1');
    ...
    public function UpdatePlayerPointsData($divisibleBy) {
        try {
            ...
            $query = $this->_connection->prepare('UPDATE loyaltydb.membercards  SET VvPreviousPoints = VvCompPoints,  VvRaffleEntry =  FLOOR(VvCompPoints/' . $divisibleBy . '), VvCompPoints = 0 WHERE Status = 1;');
...
//        $rootPath = '/var/www/HabaneroPlayerPointsUpdateCron/log/';
        $rootPath = '/var/www/cron/HabaneroPlayerPointsUpdateCron/log/';
