...
//    $vlastcron['EmailDate'] = "2016-03-02 00:00:00";  //uncomment after testing
            ...
            while ($vctr < count($vbigwinnings)) {
                ...
                if ($vterminalID <> null) {
                    ...
                    if (count($vresultd) > 0) {
                        ...
                        $bracketsize = count($varrbracketwin);
                        $ctrbracket = 0;
                        $vbracket = 0;
                        while ($ctrbracket < $bracketsize) {
                            for ($x = 1; $x < $bracketsize; $x++) {
                                $y = $x + 1;
                                if (($vnetwin >= $varrbracketwin["group" . $x]) && ($vnetwin < $varrbracketwin["group" . $y])) {
                                    $vbracket = $varrbracketwin["group" . $x];
                                    $emailgroup = $emailgroups["group" . $x];
                                    // uncomment to display 
//                                    echo "netwin >= " . $varrbracketwin["group" . $x] . " and " . "netwin < " . $varrbracketwin["group" . $y] . "\n";
                                    break;
                                }
                            }
                            if ($vnetwin >= $varrbracketwin["group" . $y]) {
                                $vbracket = $varrbracketwin["group" . $y];
                                $emailgroup = $emailgroups["group" . $y];
                                // uncomment to display condition
//                                echo "netwin >= " . $varrbracketwin["group" . $y] . "\n";
                            }
...
unset($vlastcron, $vresultrw, $vtime, $vbigwinnings, $vctr, $vterminalID, $vredeemamt, $vtranstype, $vend, $vresultd, $vresultr, $vtotalload, $vlogid, $vttype, $vreload, $vposno, $vsite, $voperator, $vterminalcode, $vstart, $vserver, $vservercode, $vnetwin, $vsiteID, $vserviceID, $vgamedesc, $vbracket, $group2, $vinserted, $vcount, $sentEmail, $ctr, $to, $subject, $message, $headers);
$obwinnings->close();
$membership->close();
$loyaltydb->close();
?>
