    ...
    protected function _reload($startSessionFormModel, $cid) {
        ...
        if ($casinoUserMode != 2) {
            if ($ref_service['Code'] == 'MM') {
                list($is_loyalty, $card_number, $loyalty, $casinos, $mid, $casinoarray_count, $isewallet) = $this->getCardInfo($loyaltyCardNo, $this->site_id, 2);
...
//                              }                          
                                ...
                                if (($casinoUserMode == 0 || $casinoUserMode == 2) && $CPV == 'v12') {
                                    ...
                                    $result = $commonUBReload->reload(toInt($this->getSiteBalance()), $amount, $paymentType, $terminal_id, $siteid, $cid, $accid, $loyaltyCardNo, $vouchercode, $trackingId, $mid, $casinoUserMode, $casinoUsername, $casinoPassword, $casinoServiceID, $traceNumber, $referenceNumber, $locatorname, $CPV);
                                } else {
...
//      $refServicesModel = new RefServicesModel();
        ...
        list($is_loyalty, $card_number, $loyalty, $casinos, $mid, $casinoarray_count, $isewallet) = $this->getCardInfo($startSessionFormModel->loyalty_card, $this->site_id, 3);
...
//        }    
    ...
    protected function _redeem($startSessionFormModel) {
        ...
        if ($service_id != NULL || $service_id != '') {
            ...
            if ($CPV != NULL || $CPV != '') {
                 ...
                 * 12/18/17
                 * Checking of Max WIthdrawn Amount       
                 *     
                 */
                $maxAmountWithdrawn = Mirage::app()->param['maxAmountWithdrawn'];
                $removeComma = str_replace(',', '', $startSessionFormModel->amount);
                $removeDot = str_replace('.00', '', $removeComma);
                $cleanAmount = $removeDot;
                if ($cleanAmount > $maxAmountWithdrawn) {
                    $message = 'Please contact Philweb Customer Service Hotline 338-3388 to Withdraw the Transaction.';
                    logger($message);
                    $this->throwError($message);
                }
                ...
                if (($ref_service['UserMode'] == 0 || $ref_service['UserMode'] == 2 || $ref_service['UserMode'] == 4) && $CPV == 'v15') {
                    ...
                    $result = $commonRedeem->redeem($login_pwd, $startSessionFormModel->terminal_id, $this->site_id, $bcf, $service_id, $startSessionFormModel->amount, $paymentType, $this->acc_id, $loyaltyCardNo, $mid, $ref_service['UserMode'], $locatorname, $casinoUsername, $casinoPassword, $casinoServiceID, $isewallet);
                    //checking if casino is terminal based
        ...
        if ($casinoUserMode != 2) {
            ...
            $transdate = CasinoApi::udate('Y-m-d H:i:s.u');
            $isSuccessful = $loyalty->processPoints($loyaltyCardNo, $transdate, 1, 'W', $startSessionFormModel->amount, $this->site_id, $result["trans_details_id"], $result['terminal_name'], $isCreditable, '', $service_id, 1);
    ...
    public function withdrawForcetAction() {
        ...
        Mirage::loadModels(array('SitesModel', 'TerminalSessionsModel', 'TerminalServicesModel',
            'RefServicesModel', 'LoyaltyRequestLogsModel', 'SpyderRequestLogsModel', 'TerminalsModel'));
        ...
        list($is_loyalty, $card_number, $loyalty, $casinos, $mid, $casinoarray_count, $isewallet, $statuscode) = $this->getCardInfo($loyaltycard, $this->site_id, 2);
...
//        $result = $pcws->Withdraw($loyaltycard, (string)$service_id, $amount, $this->site_id, $this->acc_id, $systemusername);
    ...
    protected function _closeSession($startSessionFormModel) {
        ...
        if (!empty($terminalsessiondetails)) {
            ...
            $trans_req_log_last_id = $transReqLogsModel->insert($udate, 0, 'W', 1, $startSessionFormModel->terminal_id, $this->site_id, 20, $loyalty_card, $mid, 1, '');

            $isredeemed = $commonTransactionsModel->redeemTransaction(0, $trans_summary_id, $udate, $this->site_id, $startSessionFormModel->terminal_id, 'W', 1, 20, $this->acc_id, 1, $loyalty_card, $mid);
    ...
    public function _Unlock($tCode, $cardNumber) {
        ...
        list($is_loyalty, $card_number, $loyalty, $casinos, $mid, $casinoarray_count) = $this->getCardInfo($cardNumber, $this->site_id, $terminaltype);
...
//        }
    ...
    protected function _startSession($startSessionFormModel, $return = false) {
         ...
         * Added John Aaron Vida
         * 12/19/2017
         * Check if Terminal is Activated or Not Then Blocked
         */
        if (Mirage::app()->param['isSpyderOn'] == 1) {
            $tn = substr($terminalname, 5);
            try {
                $terminalStatus = $spyderTerminalModel->checkTerminalStatus($tn);
                if ($terminalStatus == 0) {
                    $message = 'Cannot start a session on terminal that is not yet activated.';
                    logger($message);
                    $this->throwError($message);
                }
            } catch (Exception $ex) {
        ...
        if ($ref_service['UserMode'] != 2) {
            //CCT - BEGIN uncomment
            list($is_loyalty, $loyaltyCardNo, $loyalty, $casinos, $mid, $casinoarray_count, $isewallet, $statuscode) = $this->getCardInfo($startSessionFormModel->loyalty_card, $this->site_id, $terminaltype);
            // CCT - END uncomment
                            ...
                            if ($ref_service['ServiceID'] != NULL || $ref_service['ServiceID'] != '') {
                                ...
                                if ($CPV != NULL || $CPV != '') {
                                    ...
                                    if (($ref_service['UserMode'] == 0 || $ref_service['UserMode'] == 2 || $ref_service['UserMode'] == 4) && $CPV == 'v15') {
                                        ...
                                        $result = $commonUBStartSession->start($terminal_id, $siteid, 'D', $paymentType, $startSessionFormModel->casino, toInt($this->getSiteBalance()), toInt($amount), $accid, $loyaltyCardNo, $startSessionFormModel->voucher_code, $trackingId, $casinoUsername, $casinoPassword, $casinoHashedPassword, $casinoServiceID, $mid, $ref_service['UserMode'], $traceNumber, $referenceNumber, $locatorname, $CPV);
                                    } else {
...
//------------------------------------------------------------------------------------------------------------>>>>>>>>>>>>>  
    ...
    protected function _unlockSession($UnlockTerminalFormModel, $return = false) {
        ...
        list($is_loyalty, $card_number, $loyalty, $casinos, $mid, $casinoarray_count) = $this->getCardInfo($UnlockTerminalFormModel->loyalty_card, $this->site_id, $terminaltype);
        ...
        if ($ref_service['UserMode'] == 0) {
             ...
             */
            // CCT END added            
            $result = $commonStartSession->start($terminal_id, $siteid, 'D', $paymentType, $UnlockTerminalFormModel->casino, toInt($this->getSiteBalance()), toInt($amount), $accid, $card_number, $vouchercode, $trackingId, $casinoUsername, $casinoPassword, $casinoHashedPassword, $casinoServiceID, $mid, $ref_service['UserMode'], '', '', '', '');
        ...
        if ($ref_service['UserMode'] == 1) {
            ...
            $result = $commonUBStartSession->unlock($terminal_id, $siteid, 'D', $paymentType, $UnlockTerminalFormModel->casino, toInt($this->getSiteBalance()), toInt($amount), $accid, $card_number, $vouchercode, $trackingId, $casinoUsername, $casinoPassword, $casinoHashedPassword, $casinoServiceID, $mid, $ref_service['UserMode']);
        }
        ...
        $transdate = CasinoApi::udate('Y-m-d H:i:s.u');
        if ($is_loyalty) {
            $isSuccessful = $loyalty->processPoints($UnlockTerminalFormModel->loyalty_card, $transdate, 1, 'D', $amount, $siteid, $result["trans_details_id"], $result['terminal_name'], $isCreditable, $UnlockTerminalFormModel->voucher_code, 7, 1);
        }
    ...
    public function getRedeemableAmountAction() {
        ...
        list($is_loyalty, $card_number, $loyalty, $casinos, $mid, $casinoarray_count) = $this->getCardInfo($loyaltycard, $this->site_id, 2);
        ...
        list ($terminal_balance) = $casinoApi->getBalanceForceT(0, $this->site_id, 'W', 20, $this->acc_id, $casinoUsername);
    ...
    public function forceTGetBalanceAction() {
        ...
        list($is_loyalty, $card_number, $loyalty, $casinos, $mid, $casinoarray_count) = $this->getCardInfo($loyaltycard, $site_id, 2);
        ...
        list ($terminal_balance) = $casinoApi->getBalanceForceT(0, $site_id, 'W', $serviceid, $aid, $casinoUsername);
