...
//                              }                          
                                ...
                                else if(($casinoUserMode == 0 || $casinoUserMode == 2) && $CPV == 'v15')
                                {
                                    ...
                                    if (Mirage::app()->param['SkinCount'][$casinoServiceID] > 1) 
                                    {
                                        ...
                                        if ($siteclassification == 1) 
                                        { //1 - Non Platinum, 2 - Platinum
    ...
    //------------------------------------------------------------------------------------------------------------>>>>>>>>>>>>>                                         
        ...
        //          }
                    ...
                    if(($casinoUserMode == 0 || $casinoUserMode == 2) && $CPV == 'v12')
                    {
                        ...
                        $login_pwd = $terminal_pwd['HashedServicePassword'];
                        $locatorname = '';
                        $result = $commonReload->reload(toInt($this->getSiteBalance()),$amount, $paymentType, $terminal_id,
                                  $siteid,$cid,$accid,$loyaltyCardNo,$vouchercode,$trackingId, $mid,$ref_service['UserMode'],
                    ...
                    {                                     
                        ...
                        $result = $commonUBReload->reload(toInt($this->getSiteBalance()),$amount, $paymentType, $terminal_id, 
                                  $siteid,$cid,$accid,$loyaltyCardNo,$vouchercode,$trackingId, $mid, $casinoUserMode, 
                                $casinoUsername,$casinoPassword, $casinoServiceID,$traceNumber,$referenceNumber,$locatorname, $CPV);         
                    } 
                    else
                    {
                        $message = 'Error : Failed Reloading a Session.';
    ...
    //------------------------------------------------------------------------------------------------------------>>>>>>>>>>>>>
                ...
                if ($isLoyalty == 1) 
                {
                    ...
                    $loyaltyrequestlogsID = $loyaltyrequestlogs->insert($mid, 'D', $terminal_id, toInt($amount), $result["trans_details_id"], $paymentType, 1);
                    $transdate = CasinoApi::udate('Y-m-d H:i:s.u');
                    $isSuccessful = $loyalty->processPoints($loyaltyCardNo, $transdate, $paymentType, 'D', toInt($amount) ,
                                    $siteid, $result["trans_details_id"], $result['terminal_name'], 1, $vouchercode, $cid, $isCreditable);
