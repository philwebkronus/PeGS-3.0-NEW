...
class SitesModel extends MI_Model {
   ...
   * 2016-02-23
   */  
   public function getSiteAmountInfo($siteid){
        $sql = 'SELECT ROUND(LoadAmountDivisible,0) as LoadAmountDivisible FROM siteamountinfo WHERE SiteID = :site_id AND Status = 1';
        $param = array(':site_id'=>$siteid);
        $this->exec($sql,$param);
        $result = $this->find();
        $res = (!empty($result['LoadAmountDivisible']) ? $result['LoadAmountDivisible'] : 0);
        return $res;
    }
