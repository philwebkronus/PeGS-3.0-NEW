...
class TransactionRequestLogsModel extends MI_Model{
    ...
    public function insertTransactionWithBankDetails($trans_ref_id,$amount,$trans_type, $paymentType,$terminal_id,$site_id, $service_id, 
                           $loyalty_card, $mid, $user_mode, $trackingid = '',$voucher_code = '', $mg_ticket_id = '', $bankID='', $approvalCode='') {
        ...
        try {
            ...
            $stmt = $this->dbh->prepare('INSERT INTO transactionrequestlogs (TransactionReferenceID,
                                         Amount, StartDate, TransactionType, TerminalID, Status, 
                                         SiteID, ServiceID, LoyaltyCardNumber, MID, UserMode, PaymentType, 
                                         PaymentTrackingID, BankID, ApprovalCode, Option1, ServiceTransactionID)
                                         VALUES (:trans_ref_id, :amount, now(6), :trans_type, 
                                         :terminal_id, \'0\', :site_id, :service_id, :loyalty_card, 
                                         :mid, :user_mode, :paymentType ,:trackingID, :bankID, :approvalCode, :voucher_code, 
                                         :service_trans_id)');
            ...
            $stmt->bindValue(':paymentType', $paymentType);
            $stmt->bindValue(':service_trans_id', $mg_ticket_id);
            $stmt->bindValue(':bankID', $bankID);
            $stmt->bindValue(':approvalCode',$approvalCode);
    ...
    public function insertDueToZeroBalance($trans_ref_id,$amount,$trans_type,$terminal_id,
        ...
        $sql = 'INSERT INTO transactionrequestlogs (StartDate,TransactionReferenceID, Amount, 
                ...
                VALUES (now(6), :trans_ref_id, :amount, now(6), :trans_type, 
