...
class CommonReload {
    ...
    public function reload($bcf,$amount,$paymentType,$terminal_id,$site_id,$service_id,$acctid,
            $loyalty_card, $voucher_code = '',$trackingid = '', $mid = '', $userMode = '', $traceNumber='', $referenceNumber='') {
        ...
        list($terminal_balance,$service_name,$terminalSessionsModel,
                $transReqLogsModel,$redeemable_amount,$casinoApiHandler,$mgaccount) = $casinoApi->getBalance($terminal_id, $site_id,'R',$service_id);
        ...
        if ($apiresult == 'true' || $apiresult == 'TRANSACTIONSTATUS_APPROVED' || $apiresult == 'approved'){
            ...
            if(strpos($service_name, 'MG') !== false) {
                ...
                $pcwsapi->AddCompPoints($systemusername, $loyalty_card, $site_id, $service_id, number_format($amount,-2));
            }
