...
//        }
    ...
    protected function _reload($startSessionFormModel,$cid){
        ...
        if($ref_service['Code'] == 'MM'){
            ...
            if($isewallet > 0){
                $message = 'Reload Failed, Player is an ewallet account.';
                logger($message);
                $this->throwError($message);
            }
        }
...
//        $refServicesModel = new RefServicesModel();
        ...
        if($isewallet < 1){
            $message = 'Loading Failed, Player must be an ewallet account.';
            logger($message);
            $this->throwError($message);
        }
...
//                   }
    ...
    protected function _redeem($startSessionFormModel) {
        ...
        if($ref_service['Code'] == 'MM'){
            list($is_loyalty, $card_number,$loyalty, $casinos, $mid, $casinoarray_count, $isewallet) = 
                $this->getCardInfo($loyaltyCardNo, $this->site_id, 2);
            if($isewallet > 0){
                $message = 'Redemption Failed, Player is an ewallet account.';
                logger($message);
                $this->throwError($message);
            }
        }        
    ...
    public function withdrawForcetAction() {
        ...
        //verify if card is ewallet
        if($isewallet < 1){
            $message = 'Withdraw Failed, Player must be an ewallet account.';
            logger($message);
            $this->throwError($message);
        }
    ...
    public function _Unlock($tCode, $cardNumber){
        ...
        $systemusername = Mirage::app()->param['pcwssysusername'];
        $result = $pcwsAPI->UnLock($systemusername, $terminalCode, $service_id, $cardNumber);
                            ...
                            if(isset($min_deno) && $amount < toInt($min_deno)){
                                ...
                                $this->throwError($message);
                                    ...
                                    if($ref_service['Code'] == 'MM'){
                                        //verify if card is ewallet
                                        if($isewallet > 0){
                                            $message = 'Start Session Failed, Player is already an ewallet account.';
                                            logger($message);
                                            $this->throwError($message);
                                        }
                                    }
