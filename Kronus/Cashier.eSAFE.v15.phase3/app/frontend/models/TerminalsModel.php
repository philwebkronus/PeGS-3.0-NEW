...
//            GROUP BY t.TerminalCode ORDER BY CAST(`tc` AS SIGNED), TerminalID LIMIT :start,".$terminal_per_page;
    ...
    public function getForceTTerminalsToStartSession($site_id,$len) {
        $sql = "SELECT A.TerminalID TId,A.TerminalType, SUBSTR(A.TerminalCode,$len) AS tc, A.TerminalCode TCode FROM terminals A " . 
                "LEFT JOIN terminalsessions B ON A.TerminalID = B.TerminalID INNER JOIN terminalservices AS C ON A.TerminalID = C.TerminalID " . 
                "WHERE (A.TerminalType = 2) AND A.Status = '1' AND A.SiteID = :site_id  AND C.Status = 1 AND C.isCreated = 1 AND " . 
                "B.DateEnded IS NULL ORDER BY CAST(`tc` AS SIGNED), A.TerminalID";
        $param = array(':site_id'=>$site_id);
        $this->exec($sql,$param);
        return $this->findAll();
...
//        debug($ts); exit;
    ...
    public function insertserviceTransRef($service_id, $origin_id)
    {
        try {
           ...
           $smt = $this->dbh->prepare("INSERT INTO servicetransactionref (ServiceID, TransactionOrigin, DateCreated) VALUES (?, ?, now(6))");
    ...
    public function getAllActiveForceTTerminals($site_id,$len) {
        $sql = "SELECT A.TerminalID TId, A.TerminalType, SUBSTR(A.TerminalCode,$len) AS tc, A.TerminalCode TCode FROM terminals A " . 
                "INNER JOIN terminalsessions B ON A.TerminalID = B.TerminalID " . 
                "WHERE A.Status = '1' AND A.SiteID = :site_id AND A.TerminalType = 2 ORDER BY CAST(`tc` AS SIGNED), A.TerminalID";
        $param = array(':site_id'=>$site_id);
        $this->exec($sql,$param);
        $terminals = $this->findAll();
        $vips = array();
    ...
    public function getAllNotActiveForceTTerminals($site_id,$len) {
        $terminals = $this->getForceTTerminalsToStartSession($site_id, $len);
        $vips = array();
        $non_vips = array();
    ...
    public function getTerminalIDBySiteID($site_id) {
        $sql = 'SELECT TerminalID FROM terminals WHERE SiteID = :site_id';
        $param = array(':site_id'=>$site_id);
        $this->exec($sql,$param);
        $result = $this->find();
        $terminal_type = $result['TerminalID'];
     ...
     * @return array
     */
    public function getTerminalsPerRangePerSite($siteID, $siteCodeLength, $start, $end){    
        $sql = "SELECT t.TerminalID,replace(substring(replace(t.TerminalCode,'vip',''),$siteCodeLength,length(t.TerminalCode)),'VIP','') as 'Terminal',ps.Status as Session_Status, t.Status as Terminal_Status "
                . "FROM terminals t "
                . "LEFT JOIN playersessions ps ON t.TerminalID = ps.TerminalID "
                . "WHERE t.SiteID=:siteID AND replace(substring(replace(t.TerminalCode,'vip',''),$siteCodeLength,length(t.TerminalCode)),'VIP','') >=CAST(:start as SIGNED) AND replace(substring(replace(t.TerminalCode,'vip',''),$siteCodeLength,length(t.TerminalCode)),'VIP','')<=CAST(:end as SIGNED) "
                . "ORDER By t.TerminalCode";
        $param = array(':siteID'=>$siteID, ':start'=>$start, ':end'=>$end);
        $this->exec($sql,$param);
        $result = $this->findAll();
        //echo $siteCodeLength;
        //print_r($result);exit;
    ...
    public function getAllActiveTerminalPerPage2($siteid,$start,$end,$len) {
//        $sql = "SELECT t.TerminalID,SUBSTR(t.TerminalCode,$len) AS tc, t.SiteID, t.Status,t.isVIP,ts.LastTransactionDate, DATE_FORMAT(ts.LastTransactionDate,'%m/%d/%Y %H:%i:%s') as ltd,
//            TIMESTAMPDIFF(MINUTE,LastTransactionDate,NOW()) as minutes,TIMESTAMPDIFF(MINUTE,ts.DateStarted,NOW()) as dif , FORMAT(ts.LastBalance,2) as lastbalance, C.ServiceID, ts.ServiceID as usedServiceID
//            FROM terminals t LEFT JOIN terminalsessions ts ON t.terminalID = ts.terminalID INNER JOIN terminalservices AS C ON t.TerminalID = C.TerminalID " . 
//            "WHERE t.SiteID = :siteid AND C.Status = 1
//            GROUP BY t.TerminalCode ORDER BY CAST(`tc` AS SIGNED), TerminalID LIMIT :start,".$terminal_per_page;
    ...
    public function getTerminalPerPage2($siteid,$start,$end,$len) {
        
        //get all terminals under specific site
        $sql1 = "SELECT t.TerminalID,t.TerminalType,SUBSTR(t.TerminalCode,$len) AS tc,t.TerminalName, DATE_FORMAT(ts.DateStarted,'%Y-%m-%d %H:%i:%s') DateStarted, t.SiteID, t.Status,t.isVIP,ts.LastTransactionDate, DATE_FORMAT(ts.LastTransactionDate,'%m/%d/%Y %H:%i:%s') as ltd,
            TIMESTAMPDIFF(MINUTE,LastTransactionDate,NOW()) as minutes,TIMESTAMPDIFF(MINUTE,ts.DateStarted,NOW()) as dif , FORMAT(ts.LastBalance,2) as lastbalance,C.Status as ServiceStatus, C.ServiceID, ts.ServiceID as usedServiceID
            FROM terminals t LEFT JOIN terminalsessions ts ON (t.terminalID = ts.terminalID) INNER JOIN terminalservices AS C ON t.TerminalID = C.TerminalID 
            WHERE t.SiteID = :siteid  AND C.isCreated = 1 AND (SUBSTR(REPLACE(t.TerminalCode, 'VIP', ''),$len) BETWEEN $start AND $end)
            GROUP BY t.TerminalCode";
     ...
     public function getLastTerminalCodeBySiteID($siteid,$len) {

        $sql = "SELECT REPLACE(SUBSTR(TerminalCode,$len),'VIP', '') AS TerminalCode FROM terminals WHERE SiteID=:siteid ORDER BY TerminalCode DESC LIMIT 1";        
        $param = array(':siteid'=>$siteid);
        $this->exec($sql,$param);
        return $this->find();
    }
