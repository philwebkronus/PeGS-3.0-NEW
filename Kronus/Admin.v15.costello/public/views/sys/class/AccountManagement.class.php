...
//      }
      ...
      function viewlimitaccounts($zaccID, $zAcctType, $zStart, $zLimit, $zpegs, $sidx, $sord)
      {
         if($zaccID > 1)
         {
             $stmt = "Select a.AID, (CASE a.Status WHEN '0' THEN 'Inactive' 
                                                   WHEN '1' THEN 'Active' 
                                      ...
                                      END) as Status, 
                                      a.UserName,a.AccountTypeID, b.Name, b.Email, b.Address 
                    from accounts as a 
                        INNER JOIN accountdetails as b ON a.AID = b.AID AND a.AID = '".$zaccID."'
         ...
         else
         {
            ...
            if($zpegs == 0)
            {
               $stmt = "Select a.AID, (CASE a.Status WHEN '0' THEN 'Inactive' 
                         ...
                         from accounts as a 
                         INNER JOIN accountdetails as b ON a.AID = b.AID 
                            WHERE a.AccountTypeID = '".$zAcctType."'
                                ORDER BY $sidx $sord  LIMIT ".$zStart.", ".$zLimit."";
            }
            elseif($zAcctType > 0 && count($zpegs) > 0)
            {
                ...
                $stmt = "Select a.AID, (CASE a.Status WHEN '0' THEN 'Inactive' 
                          ...
                          WHERE a.AccountTypeID = '".$zAcctType."' AND c.SiteID IN(".$pegs.") ORDER BY $sidx $sord  LIMIT ".$zStart.", ".$zLimit."";
            }
            else
            {
                 $stmt = "Select a.AID, (CASE a.Status WHEN '0' THEN 'Inactive' 
                                                    ...
                                                    WHEN '6' THEN 'Password Expired' 
                                                    WHEN '5' THEN 'Terminated' 
                                      END) as Status, a.UserName,a.AccountTypeID, b.Name, b.Email, b.Address 
                            from accounts as a 
                                INNER JOIN accountdetails as b ON a.AID = b.AID
                                    ORDER BY $sidx $sord LIMIT ".$zStart.", ".$zLimit."";
            }
    ...
    function getOptrStatus($zAID)
     ...
     function getActiveOperator($zacctypeid){
         $stmt = "SELECT UserName, AID FROM accounts WHERE AccountTypeID = ? 
                  AND Status IN (1,6) 
