...
$oaccount2 = new AccountManagement($_DBConnectionString[4]);
$oaccount3 = new AccountManagement($_DBConnectionString[5]);
$connected = $oaccount->open();
$connected2 = $oaccount2->open();
$connected3 = $oaccount3->open();
$nopage = 0;
if($connected)
{
/************ SESSION CHECKING **************/
...
//   $_SESSION['liason'] = $rnoliason;
   ...
   if(isset ($_POST['TerminatePage']) == "TerminateAccount")
   {
       ...
       switch($_SESSION['acctype'])
       {
         case 1:
               //admin
               $allaccs = $oaccount->viewlimitaccounts(0, $vacctype, $start, $limit, 0, $sort, $sord);
         break;
         case 2:
               //Operator
              $allaccs = $oaccount->viewlimitaccounts(0, $vacctype, $start, $limit, $_SESSION['pegsowned'], $sort, $sord);
         break;
         case 8:
               //PEGS Ops
              $allaccs = $oaccount->viewlimitaccounts(0, $vacctype, $start, $limit, 0, $sort, $sord);
          ...
          */
         default:
               $allaccs = $oaccount->viewlimitaccounts(0, 0, $start, $limit, 0, $sort, $sord);
       }
       if(count($allaccs) > 0)
       {
         $i = 0;
