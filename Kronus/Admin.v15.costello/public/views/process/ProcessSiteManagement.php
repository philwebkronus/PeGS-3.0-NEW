...
if ($connected)
{
    ...
    $resultsiteclass = array();
    $resultsiteclass = $osite->showsiteclass();
    $resultsitegrp = array();
    ...
    if (isset($_POST['page']))
    {
        ...
        {
            case 'SiteCreation':
                if (isset($_POST['txtsitecode']) && isset($_POST['cmbislands']) && isset($_POST['cmbregions']) && isset($_POST['cmbprovinces']) && isset($_POST['cmbcity']) && isset($_POST['cmbbrgy']) && isset($_POST['txtsiteaddress']) && isset($_POST['txtpasscode']) && isset($_POST['tmtab']) && isset($_POST['srrtab']) && isset($_POST['esafetab']))
                {
                    ...
                    $siteClassification = $_POST['cmbsiteclass'];
                    ...
                    $vtmtab = $_POST['tmtab']; //indicates if terminal monitoring tab is shown
                    $vsrrtab = $_POST['srrtab']; //indicates if start session, reload session and redemption tab is shown
                    $vesafetab = $_POST['esafetab']; //indicates if s-SAFE load and withdraw tab is shown
                    ...
                    else
                    {
                        ...
                        $siteid = $osite->insertinsite($vSiteName, $vSiteCode, $vOwnerAID, $vStatus, $vSiteDescription, $vSiteAlias, $vIslandId, $vRegionID, $vProvinceID, $vCityID, $vBarangayID, $vSiteAddress, $vsiteCTO, $rpass, $vdate, $aid, $rdenomdefaults, $istestsite, $vcontactno, $vtmtab, $vsrrtab, $vesafetab, $siteClassification);
                        if ($siteid == 0)
                        {
                        ...
                        else
                        {
                            ...
                            $resultid0 = $osite->insertaccount($vUserName[0], $vPassword, $vAccountTypeID[0], $vPasskey, $vStatus, $vAccountGroupID, $vDateLastLogin, $vLoginAttempts, $vSessionNoExpire, $vDateCreated, $vCreatedByAID, $vForChangePassword, $vWithPasskey, $vAID, $vName, $vAddress, $vEmail[0], $vLandline, $vMobileNumber, $vOption1, $vOption2, $vdesignationID, $vSiteID, $vdateissued, $vdateexpires);

                            $resultid = $osite->insertaccount($vUserName[1], $vPassword, $vAccountTypeID[1], $vPasskey, $vStatus, $vAccountGroupID, $vDateLastLogin, $vLoginAttempts, $vSessionNoExpire, $vDateCreated, $vCreatedByAID, $vForChangePassword, $vWithPasskey, $vAID, $vName, $vAddress, $vEmail[1], $vLandline, $vMobileNumber, $vOption1, $vOption2, $vdesignationID, $vSiteID, $vdateissued, $vdateexpires);
                    ...
                    unset($rdenomdefaults, $vmessage, $rlocation, $arrisland, $arrregion, $arrprovince, $arrcity, $arrbrgy, $arrAID, $rsite);
                }
                else
                {
            ...
            case 'SiteUpdate':
                ...
                $istestsite = $_POST['opttest']; //indicates if site is test or live
                $siteClassification = $_POST['cmbsiteclass']; //indicate id the site is tagged platinum or non platinum
...
//               {
                ...
                if ($terminalSessionCount == 0)
                {
                    ...
                    {
                        $siteid = $osite->updatesitedetails($vSiteID, $vSiteName, $vSiteCode, $vOwnerAID, $vSiteGroupID, $vSiteDescription, $vSiteAlias, $vIslandId, $vRegionID, $vProvinceID, $vCityID, $vBarangayID, $vSiteAddress, $vCTO, $vpasscode, $vstatus, $voldownerAID, $istestsite, $vcontactno, $vtmtab, $vsrrtab, $vesafetab);
                    }
                    else
                    {
                        $siteid = $osite->updatesitedetails2($vSiteID, $vSiteName, $vSiteCode, $vOwnerAID, $vSiteGroupID, $vSiteDescription, $vSiteAlias, $vIslandId, $vRegionID, $vProvinceID, $vCityID, $vBarangayID, $vSiteAddress, $vCTO, $vpasscode, $vstatus, $voldownerAID, $istestsite, $vcontactno, $vtmtab, $vsrrtab, $vesafetab, $siteClassification);
                    }
                }
                else
                {
...
//               }
            ...
            case 'UpdateStatus':
                ...
                if ($vStatus == 1)
                {
                    ...
                    if (count($checkcashier) == 0 || count($checkcashier) == 1)
                    {
                        ...
                        if ($CreateEGMVC)
                        {
                            ...
                            $resultid = $osite->insertaccount($vUserName, $vPassword, $vAccountTypeID, $vPasskey, $vStatus, $vAccountGroupID, $vDateLastLogin, $vLoginAttempts, $vSessionNoExpire, $vDateCreated, $vCreatedByAID, $vForChangePassword, $vWithPasskey, $vAID, $vName, $vAddress, $vEmail, $vLandline, $vMobileNumber, $vOption1, $vOption2, $vdesignationID, $vSiteID, $vdateissued, $vdateexpires);
                        }
                ...
                else
                {
                    ...
                    if ($resultid > 0)
                    {
                        ...
                        $osite->emailalerts($vtitle, $grouppegs, $vmessage);
                        unset($arrsite, $resultid, $vsitecode, $vdateupdated, $vtransdetails, $vtitle, $arraid, $raid, $dateformat, $vmessage);
                    }
                    else
                    {
            ...
            case 'SiteDenomination':
                ...
                case 'updateloadamt':
                $vsite = $_POST['cmbsite'];
                $vloadamt = $_POST['txtloadamt'];
                $vAID = $_POST['AID'];
                $checkSiteAmount = $osite->chksiteamount($vsite);
                $sitecount = $checkSiteAmount[0]['siteCount'];  
                $siteAmtID = $checkSiteAmount[0]['SiteAmountID'];
                $loadamtdivisble = $checkSiteAmount[0]['LoadAmountDivisible'];
                if ($loadamtdivisble!=$vloadamt)
                {
                    if ($sitecount<1)
                    {
                        $insertSiteAmt = $osite->insertsiteamount($vsite,$vloadamt,$vAID);
                        $vtransdetails = "Insert Site Amount";
                        $vauditfuncID = "108";
                        $osite->logtoaudit($new_sessionid, $aid, $vtransdetails, $vdate, $vipaddress, $vauditfuncID); //insert in audittrail
                        $msg = "Site Amount Successfully Added";
                            $_SESSION['mess'] = $msg;
                    }
                    else 
                    {
                        $updateSiteAmt = $osite->updatesiteamount($vsite,$vloadamt,$vAID,$siteAmtID);
                        $vtransdetails = "Update Site Amount";
                        $vauditfuncID = "109";
                        $osite->logtoaudit($new_sessionid, $aid, $vtransdetails, $vdate, $vipaddress, $vauditfuncID); //insert in audittrail
                        $msg = "Site Amount Successfully Updated";
                            $_SESSION['mess'] = $msg;
                    }
                }
                else 
                {
                    $msg = "Site Amount Unchanged";
                            $_SESSION['mess'] = $msg;
                }
                header("Location: ../updateSiteAmount.php");
                break;
            default:
                if ($nopage == 0)
                {
    ...
    elseif (isset($_GET['page']) == 'ViewSite')
    {
        ...
        if (count($resultsite) > 0)
        {
            ...
            $arrdemographics = array("Regions" => $rregions, "Provinces" => $rprovince, "Cities" => $rcities, "Barangays" => $rbarangays, "Site Classification" => $resultsiteclass);
    ...
    elseif (isset($_POST['siteamt']))
    {
        $vsiteID = $_POST['siteamt'];
        $rresult = array();
        $rresult = $osite->getsiteamt($vsiteID);

        foreach ($rresult as $row)
        {
            $rloadamt = number_format($row['LoadAmountDivisible'], 0);
            
        }
        if (count($rresult) > 0)
        {
            $vloadamt->LoadAmt = $rloadamt;
        }
        else
        {
            $vloadamt->LoadAmt = "";
        }
        echo json_encode($vloadamt);
        unset($rresult);
        $osite->close();
        exit;
    }
    else
    {
        ...
        $_SESSION['siteclass'] = $resultsiteclass;
