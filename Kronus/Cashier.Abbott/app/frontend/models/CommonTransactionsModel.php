...
class CommonTransactionsModel extends MI_Model{
     ...
     */
     public function startTransaction($site_id,$terminal_id,$amount,$acctid, $trans_ref_id,
                                      $trans_type, $paymentType,$service_id,$trans_status, $loyalty_card, $mid)
    {
        $beginTrans = $this->beginTransaction();
        
        try{
            $stmt = $this->dbh->prepare('INSERT INTO transactionsummary 
                                        (SiteID, TerminalID, Deposit, DateStarted, 
                                         DateEnded, CreatedByAID, LoyaltyCardNumber,MID) 
                                        VALUES (:site_id, :terminal_id, :amount, now_usec(), 
                                                \'0\', :acctid, :loyalty_card, :mid)');
            ...
            if($stmt->execute()) {
                ...
                $stmt = $this->dbh->prepare('INSERT INTO transactiondetails (TransactionReferenceID, 
                        ...
                        now_usec(), :service_id, :acct_id, :trans_status, :loyalty_card, :mid, :paymentType)');
        ...
        try{
            ...
            if($stmt->execute()) {
                $stmt = $this->dbh->prepare('INSERT INTO transactiondetails (TransactionReferenceID, 
                                             ...
                                             :amount, now_usec(), :service_id, :acct_id, :trans_status, :loyalty_card, :mid, :paymentType)');
                ...
                if($stmt->execute()) {
                    ...
                    $stmt = $this->dbh->prepare('UPDATE terminalsessions SET ServiceID = :service_id, 
                                                 LastBalance = :terminal_balance, LastTransactionDate = now_usec() 
                                                 WHERE TerminalID = :terminal_id');
                    $stmt->bindValue(':service_id', $service_id);
