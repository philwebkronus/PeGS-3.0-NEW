                ...
                for (var i = 0; i < data.terminals.length; i++) {
                    ...
                    if (cntr % 2 == 0) {
                        ...
                        if (data.terminals[i + 1] != undefined) {
                            ...
                            var casinovip = data.services[vipid];
                            casinononvip = data.services[nonvipid];
                        }
                        ...
                        } else if (data.terminals[i + 1]['usedServiceID'] != null || data.terminals[i]['usedServiceID'] != null && (data.terminals[i + 1]['Status'] == 1 && data.terminals[i]['Status'] == 1)) {
//                            casino = getTimePlaying(data.terminals[i]['minutes'])+' - '+data.refservices[data.terminals[i]['ServiceID']];
                            if (data.terminals[i]['usedServiceID'] != null) {
                                ...
                                if (data.terminals[i]['lastbalance'] != null) {
                                    last_balance = 'PhP ' + data.terminals[i]['lastbalance'];
                                } else {
                                    last_balance = 'PhP N/A';
                                }
                            } else {
                                // check if vip
                                if (data.terminals[i + 1]['usedServiceID'] != null) {
...
//                                casino = getTimePlaying(data.terminals[i + 1]['minutes'])+' - '+data.refservices[data.terminals[i + 1]['ServiceID']];
                            ...
                            cls = 'active';
//                            if(data.terminals[i]['TerminalType'] == 2){
//                                button='<input tid="'+tid+'" cid="'+cid+'" type="button" value="LOCK" class="lock" style="width:100%;"/>';
//                            }
//                            else{
...
//                            if(data.terminals[i]['TerminalType'] == 2){
//                                button = '<input class="unlock" type="button" value="UNLOCK"/>';
//                            }
//                            else{
...
//                            }
                        ...
                        html += '<div class="box chk">' + checkbox + '</div>';
