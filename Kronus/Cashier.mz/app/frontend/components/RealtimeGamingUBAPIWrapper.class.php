...
Mirage::loadComponents(array('CasinoAPI/RealtimeGamingCashierAPI2.class',
    ...
    'CasinoAPI/RealtimeGamingWCFPlayerAPI.class'));
...
class RealtimeGamingUBAPIWrapper {
    ...
    public function __construct($URI, $API, $certFilePath, $keyFilePath, $caching = '') {
        switch ($API) {
            ...
            case self::PLAYER_API2: {
                    $this->_API = new RealtimeGamingWCFPlayerAPI($URI, $certFilePath, '');
                    break;
                }
        }
    }
    ...
    public function ChangePlayerClassification($pid, $playerClassID) {
        $response = $this->_API->changePlayerClasification($pid, $playerClassID);

        if (!$this->_API->GetError()) {
            if (!is_null($response['ChangePlayerClassResult'])) {
                if (is_array($response)) {
                    return array('IsSucceed' => true, 'ErrorCode' => 0, 'ErrorMessage' => null, 'ChangePlayerClassResult' => $response);
                } else {
                    return array('IsSucceed' => false, 'ErrorCode' => 30, 'ErrorMessage' => 'Response malformed');
                }
            } else {
