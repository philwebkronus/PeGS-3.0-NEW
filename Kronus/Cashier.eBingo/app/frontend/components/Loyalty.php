...
class Loyalty {
    public function getCardInfo($card_number,$return_transfer=false) {
        $ch = curl_init();
        curl_setopt($ch,CURLOPT_URL, Mirage::app()->param['card_inquiry'] . '?SerialNumber=' . $card_number);
...
//        $trans_date = date('Y-m-d H:i:s', $trans_date);
        ...
        return Mirage::app()->param['add_points'] . '?sessionid=' . $session_id . '&cardnumber=' . $card_number . '&transdate=' . $trans_date . 
...
//        $trans_date = urlencode($trans_date);
        return Mirage::app()->param['add_points'] . '?sessionid=' . $session_id . '&transdate=' . $trans_date . '&amount=' . $amount . 
    ...
    public function getUrlWithdraw($session_id,$trans_date,$amount,$pos_account_no,$service_id,$terminal_login,$provider_trans_id) {
        ...
        if(count($amount) == 2)
            ...
            $amount = $old;
//        //$stringdate = strtotime(substr('20120214194445815067',0,14));
//        $trans_date = strtotime($trans_date);
//        $trans_date = date('Y-m-d H:i:s', $trans_date);
//        $trans_date = urlencode($trans_date);
        return Mirage::app()->param['withdraw'] . '?sessionid=' . $session_id . '&transdate=' . $trans_date . '&amount=' . $amount . 
                '&posaccountno=' . $pos_account_no . '&serviceid=' . $service_id . '&terminallogin=' . $terminal_login . 
                '&providertransactionid=' . $provider_trans_id;
    }    
}
