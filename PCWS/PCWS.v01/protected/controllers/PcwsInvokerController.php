...
class PcwsInvokerController extends CController{
    ...
    public function actionDeposit(){
        ...
        if(isset($_POST['ServiceID']) && isset($_POST['CardNumber']) && isset($_POST['Amount']) && isset($_POST['PaymentType']) && isset($_POST['SiteID']) && isset($_POST['AID']) && isset($_POST['AccessDate']) && isset($_POST['Token'])){
            ...
            $result = $this->_deposit($serviceid, $cardnumber, $amount, $paymenttype, $siteid, $aid, $username,$accessdate,$tkn,$tracenumber,$referencenumber);
        }
    ...
    private function _deposit($serviceid, $cardnumber, $amount, $paymenttype, $siteid, $aid, $username,$accessdate,$tkn,$tracenumber,$referencenumber){
        ...
        $postdata = CJSON::encode(array('ServiceID'=>$serviceid, 'CardNumber'=>$cardnumber, 'Amount'=>$amount, 'PaymentType'=>$paymenttype, 'SiteID'=>$siteid, 'AID'=>$aid,
                                                                            'SystemUsername'=>$username, 'AccessDate'=>$accessdate, 'Token'=>$tkn, 'TraceNumber'=>$tracenumber, 'ReferenceNumber'=>$referencenumber));
