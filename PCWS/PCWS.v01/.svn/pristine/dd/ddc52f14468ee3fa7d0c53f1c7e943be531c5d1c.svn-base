...
class RealtimeGamingCashierAPI2
{
    ...
    public function CreateToken( $pid )
    {	
        $data = array( 'PID' => $pid );
        
        $response = $this->SubmitRequest( $this->_url . '/CreateToken', http_build_query( $data ) );
        ...
        {                        
            ...
            if(count($this->_APIresponse) == 0)
            {
              $this->_APIresponse = array( 'CreateTokenResult' => $this->_APIresponse);
            }
            
            else{
              $this->_APIresponse = array( 'CreateTokenResult' => $this->_APIresponse[0]);    
            }
        }
        else
        {
