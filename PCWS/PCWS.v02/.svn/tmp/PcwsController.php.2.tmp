...
//                                    $hours = $hours + ($diff->days*24);
    ...
    public function actionForceLogout(){
        ...
        $transactionMessage = array(
            ...
            21=>'Card does not have an existing session',
            8=>'Can\'t get balance',
            19=>'Failed to end session',
            26=>"Player's account must be e-SAFE."
        );
                                ...
                                if($AID){
                                    ...
                                    if(is_array($balances) && !empty($balances)){
                                        ...
                                        if($isWallet==1){
                                            ...
                                            $eCode = 26;//Player must be an ewallet account.
                                        }
                                    }else{
                                        $eCode = 8;// Can't get balance.
                                    }
    ...
    public function actionCreateSession(){
        ...
        $transactionMessage = array(
            null=>'',
            0=>'Transaction successful, Terminal Session has been started.',
            4=>'All fields are required',
            5=>'Card not found',
            6=>'Card does not have existing casino account',
            8=>'Can\'t get balance',
    ...
    public function actionRemovesession(){
        ...
        $transactionMessage = array(
            null=>'',
            0=>'Transaction successful, Terminal session has been successfully removed.',
            4=>'Incomplete data / Invalid data',
            21=>'Card does not have an existing session',
            8=>'Can\'t get balance',
            19=>'Failed to end session',
            26=>"Player's account must be e-SAFE", 
            24=>'This process is not applicable for sessions that has transactions. '
        );
        ...
        if($isconnvalid == 0){
            ...
            if(isset($data['TerminalCode']) && isset($data['CardNumber']) && isset($data['SystemUsername'])
                && isset($data['AccessDate']) && isset($data['AccessDate'])){
                ...
                $transMessage = $transactionMessage[$eCode];
                $data = CommonController::removeSession($transMessage, $eCode);
        }
        else {
            switch ($isconnvalid) {
                case 1:
    ...
    public function actionUnlockgenesis(){
        ...
        if($isconnvalid == 0){
            ...
            if(isset($data['TerminalCode']) && isset($data['ServiceID'])
                ...
                $transMessage = $transactionMessage[$eCode];
                $data = CommonController::unlockgenesis($transMessage, $eCode);
    ...
    public function actionForceLogoutGen(){
        ...
        $transactionMessage = array(
            ...
            8=>'Can\'t get balance',
            19=>'Failed to end session',
            22=>'Player must be an e-SAFE account.', 
            26=>'Player has no EGM session.'
        );
        ...
        if($isconnvalid == 0){
                ...
                if(isset($cardnumber) && $cardnumber != "" && isset($password) && $password != "" && isset($pin) && $pin != "" && isset($confirmpin) && $confirmpin != "" && isset($this->_dt) && $this->_dt != "" && isset($this->_tkn) && $this->_tkn != "")
                {
                    ...
                    if($isesafe == 0)
                    {
                            ...
                            if($iswhitelisted == 1)
                            {
                                ...
                                if($ispwvalid == 1)
                                {
                                    ...
                                    if(is_numeric($pin) && strlen($pin) == 6)
                                    {
                                        ...
                                        {
                                            $errCode = 4;
                                            $transMsg = 'Invalid data. Pin is different from confirm pin.';
                                        }
                                    }
                                    else
                                    {
                                        $errCode = 4;
                                        $transMsg = 'Invalid data. Pin is not exactly 6 digits long.';
                                    }
                                }
                                else
                                {
                                    $errCode = 4;
                                    $transMsg = 'Invalid data. Password is invalid.';
                                }
                            }
                            else
                            {
                        ...
                        {
                            $errCode = 4;
                            $transMsg = 'Invalid data. Card must be active.';
                        }
                    }
                    else 
                    {
